		...
		<div class="slos-dashboard-card">
			...
			<div class="slos-card-body">
				...
				<div class="slos-compliance-badges">
					<div class="slos-compliance-badge <?php echo esc_attr( $average_score >= 85 ? 'active' : 'inactive' ); ?>">
						<span class="dashicons dashicons-yes-alt"></span>
						<span><?php esc_html_e( 'ADA', 'shahi-legalflowsuite' ); ?></span>
					</div>
					<div class="slos-compliance-badge <?php echo esc_attr( $average_score >= 80 ? 'active' : 'inactive' ); ?>">
						<span class="dashicons dashicons-yes-alt"></span>
						<span><?php esc_html_e( 'Section 508', 'shahi-legalflowsuite' ); ?></span>
					</div>
					<div class="slos-compliance-badge <?php echo esc_attr( $average_score >= 90 ? 'active' : 'inactive' ); ?>">
						<span class="dashicons dashicons-yes-alt"></span>
				...
				<div class="slos-history-table-container">
					...
					<div class="slos-history-pagination">
						<button type="button" class="slos-load-more-btn" id="slos-load-more-history">
							<?php
							$remaining_scans = max( 0, count( $scan_history ) - 10 );
							/* translators: %d: number of remaining scan history items */
							printf( esc_html__( 'Load more (%d remaining)', 'shahi-legalflowsuite' ), absint( $remaining_scans ) );
							?>
						</button>
					</div>
					<?php endif; ?>
						...
						<div class="slos-trend-header">
							<div>
								...
								<p class="slos-trend-subtitle">
									<?php
									$days_shown = absint( min( 30, count( $scan_history ) ) );
									/* translators: %d: number of scans shown */
									printf( esc_html__( 'Last %d scans', 'shahi-legalflowsuite' ), esc_html( $days_shown ) );
									?>
								</p>
							</div>
...
<script>
const slosExportNonce = <?php echo wp_json_encode( wp_create_nonce( 'slos_export_nonce' ) ); ?>;
