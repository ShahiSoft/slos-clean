...
(function() {
    ...
    const SLOSAccessibilityFixes = {
         ...
         */
        initKeyboardTrapHandlers: function() {
            ...
            document.querySelectorAll('[data-slos-escape-trigger]').forEach(btn => {
         ...
         */
        closeParentDialog: function(button) {
            const dialog = button.closest('[role="dialog"], [role="alertdialog"], .modal, .popup');
            if (dialog) {
        ...
        toggleAnimation: function(button) {
            ...
            const animated = container.querySelector('[data-animated], [data-slos-animation-state]');
            ...
            if (isPaused) {
                ...
                if (animated) {
                    ...
                    animated.setAttribute('data-slos-animation-state', 'running');
                }
                ...
                if (animated) {
                    ...
                    animated.setAttribute('data-slos-animation-state', 'paused');
                }
         ...
         */
        toggleCarousel: function(button) {
         ...
         */
        triggerCarouselEvent: function(carousel, action) {
            // Slick
            if (carousel.classList.contains('slick-slider') && typeof jQuery !== 'undefined') {
                jQuery(carousel).slick(action === 'pause' ? 'slickPause' : 'slickPlay');
            }
            // Swiper
            if (carousel.swiper) {
                carousel.swiper.autoplay[action === 'pause' ? 'stop' : 'start']();
            }
            // Owl Carousel
            if (typeof jQuery !== 'undefined' && jQuery(carousel).data('owl.carousel')) {
                jQuery(carousel).trigger(action === 'pause' ? 'stop.owl.autoplay' : 'play.owl.autoplay');
            }
        ...
        pauseAllAnimations: function() {
            ...
            // Pause marquees
            document.querySelectorAll('.slos-accessible-marquee').forEach(marquee => {
                marquee.setAttribute('data-paused', 'true');
            });
        },
         ...
         */
        initTimingControls: function() {
            // Time extension buttons
            document.querySelectorAll('.slos-extend-time').forEach(btn => {
                btn.addEventListener('click', (e) => this.extendTime(e.currentTarget));
            });

            // Cancel refresh buttons
            document.querySelectorAll('.slos-cancel-refresh').forEach(btn => {
                btn.addEventListener('click', (e) => this.cancelRefresh(e.currentTarget));
            });

            // Session extend buttons
            document.querySelectorAll('.slos-extend-session').forEach(btn => {
                btn.addEventListener('click', (e) => this.extendSession(e.currentTarget));
            });

            // Timer pause buttons
            document.querySelectorAll('.slos-timer-pause').forEach(btn => {
                btn.addEventListener('click', (e) => this.toggleTimerPause(e.currentTarget));
            });

            // Timer add time buttons
            document.querySelectorAll('.slos-timer-add').forEach(btn => {
                btn.addEventListener('click', (e) => this.addTimerTime(e.currentTarget));
            });

            // Keep visible buttons (for auto-dismiss notifications)
            document.querySelectorAll('.slos-keep-visible').forEach(btn => {
                btn.addEventListener('click', (e) => this.keepNotificationVisible(e.currentTarget));
            });

            // Notification close buttons
            document.querySelectorAll('.slos-notification-close').forEach(btn => {
                btn.addEventListener('click', (e) => this.closeNotification(e.currentTarget));
            });

            // Pause auto-advance buttons
            document.querySelectorAll('.slos-pause-advance').forEach(btn => {
                btn.addEventListener('click', (e) => this.toggleAutoAdvance(e.currentTarget));
            });
        },
         ...
         */
        extendTime: function(button) {
            const seconds = parseInt(button.getAttribute('data-extend-seconds'), 10) || this.config.defaultTimeExtension;
            
            // Dispatch event for any listening refresh handlers
            document.dispatchEvent(new CustomEvent('slos-extend-time', {
            ...
            this.announce('Time extended by ' + Math.floor(seconds / 60) + ' minutes');
        },

        /**
         * Cancel page refresh
         */
        cancelRefresh: function(button) {
            // Remove meta refresh
            const metaRefresh = document.querySelector('meta[http-equiv="refresh"]');
            if (metaRefresh) {
                metaRefresh.remove();
        ...
        extendSession: function(button) {
            ...
            if (typeof slosa11yConfig !== 'undefined' && slosa11yConfig.ajaxUrl) {
                fetch(slosa11yConfig.ajaxUrl, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: 'action=slos_extend_session&nonce=' + (slosa11yConfig.nonce || '')
                }).catch(() => {});
            }
         ...
         */
        toggleTimerPause: function(button) {
            const container = button.closest('[data-slos-timing-control]');
            const timer = container ? container.querySelector('[role="timer"]') : null;
         ...
         * Add time to timer
         */
        addTimerTime: function(button) {
            const seconds = parseInt(button.getAttribute('data-add-seconds'), 10) || 60;
            const container = button.closest('[data-slos-timing-control]');
            const timer = container ? container.querySelector('[role="timer"]') : null;

            if (timer) {
                timer.dispatchEvent(new CustomEvent('slos-timer-add', {
                    detail: { seconds: seconds }
                }));
            }
         ...
         */
        keepNotificationVisible: function(button) {
         ...
         * Close notification
         */
        closeNotification: function(button) {
            const notification = button.closest('[data-slos-timing-control], .notification, .alert, .notice');
            if (notification) {
         ...
         */
        toggleAutoAdvance: function(button) {
            ...
            if (isPaused) {
                ...
                if (wizard) {
                    ...
                    wizard.dispatchEvent(new CustomEvent('slos-advance-resume'));
                }
        ...
        initFocusManagement: function() {
            ...
            document.addEventListener('focusin', (e) => {
                if (!e.target.closest('[role="dialog"], [role="alertdialog"]')) {
         ...
         */
        initFocusVisiblePolyfill: function() {
            ...
            const isValidFocusTarget = (el) => {
                if (el.tagName === 'INPUT' && inputTypesAllowlist[el.type] && !el.readOnly) {
                    return true;
                }
        ...
        getFocusableElements: function(container) {
            return Array.from(container.querySelectorAll(this.config.focusableSelector))
         ...
         */
        initARIALiveAnnouncer: function() {
            ...
            let assertiveAnnouncer = document.getElementById('slos-a11y-announcer-assertive');
            if (!assertiveAnnouncer) {
                ...
                assertiveAnnouncer.style.cssText = 'position:absolute;left:-10000px;width:1px;height:1px;overflow:hidden;';
         ...
         */
        initReducedMotionListener: function() {
            ...
            const handleChange = (e) => {
                ...
                if (e.matches) {
                    this.pauseAllAnimations();
                    this.announce('Animations paused due to reduced motion preference');
                }
            };
        ...
        initModalFocusTraps: function() {
            document.querySelectorAll('[data-slos-escape-enabled][role="dialog"], [data-slos-escape-enabled][role="alertdialog"]').forEach(dialog => {
                ...
                mutations.forEach(mutation => {
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            if (node.matches && node.matches('[data-slos-escape-enabled][role="dialog"]')) {
                                this.setupFocusTrap(node);
                            }
                            const dialogs = node.querySelectorAll && node.querySelectorAll('[data-slos-escape-enabled][role="dialog"]');
                            if (dialogs) {
         ...
         */
        setupFocusTrap: function(dialog) {
