<?php
/**
 * Canonical Fixer/Checker ID Map
 *
 * Single source of truth for all fixer and checker IDs.
 * Eliminates aliases and establishes one-to-one mapping.
 *
 * @package ShahiLegalFlowSuite
 * @subpackage FixEngine
 * @since 3.1.2
 */

namespace ShahiLegalFlowSuite\FixEngine;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * Canonical ID Registry
 *
 * Maps canonical IDs to metadata (name, severity, category, WCAG).
 */
class CanonicalIds {

	/**
	 * Severity levels
	 */
	const SEVERITY_CRITICAL = 'critical';
	const SEVERITY_SERIOUS  = 'serious';
	const SEVERITY_MODERATE = 'moderate';
	const SEVERITY_MINOR    = 'minor';

	/**
	 * Categories
	 */
	const CAT_IMAGES         = 'images';
	const CAT_HEADINGS       = 'headings';
	const CAT_LINKS          = 'links';
	const CAT_FORMS          = 'forms';
	const CAT_TABLES         = 'tables';
	const CAT_MEDIA          = 'media';
	const CAT_INTERACTIVITY  = 'interactivity';
	const CAT_COLOR_CONTRAST = 'color-contrast';
	const CAT_TOUCH_VIEWPORT = 'touch-viewport';
	const CAT_ARIA           = 'aria';
	const CAT_STRUCTURE      = 'structure';
	const CAT_CONTENT        = 'content';

	/**
	 * Canonical ID map
	 *
	 * Key: Canonical ID (used everywhere)
	 * Value: Metadata array
	 */
	private static $ids = array(

		// ========== IMAGES ==========
		'missing-alt-text'         => array(
			'name'         => 'Missing Alt Text',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'empty-alt-text'           => array(
			'name'         => 'Empty Alt Text',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'redundant-alt-text'       => array(
			'name'         => 'Redundant Alt Text',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'decorative-image'         => array(
			'name'         => 'Decorative Image',
			'severity'     => self::SEVERITY_MINOR,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'page-title'               => array(
			'name'         => 'Page Title',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_STRUCTURE,
			'wcag'         => array( '2.4.2' ),
			'auto_fixable' => true,
		),
		'lang-attribute'           => array(
			'name'         => 'Language Attribute',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '3.1.1', '3.1.2' ),
			'auto_fixable' => true,
		),
		'button-type'              => array(
			'name'         => 'Button Type',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'table-scope'              => array(
			'name'         => 'Table Scope',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'figure-caption'           => array(
			'name'         => 'Figure Caption',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'list-structure'           => array(
			'name'         => 'List Structure',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_STRUCTURE,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'complex-image'            => array(
			'name'         => 'Complex Image',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => false,
		),
		'svg-accessibility'        => array(
			'name'         => 'SVG Accessibility',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'background-image'         => array(
			'name'         => 'Background Image',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => false,
		),
		'logo-image'               => array(
			'name'         => 'Logo Image',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => true,
		),
		'image-map-alt'            => array(
			'name'         => 'Image Map Alt',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1', '2.4.4' ),
			'auto_fixable' => true,
		),
		'alt-text-quality'         => array(
			'name'         => 'Alt Text Quality',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_IMAGES,
			'wcag'         => array( '1.1.1' ),
			'auto_fixable' => false,
		),

		// ========== HEADINGS ==========
		'missing-h1'               => array(
			'name'         => 'Missing H1',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '2.4.6' ),
			'auto_fixable' => true,
		),
		'multiple-h1'              => array(
			'name'         => 'Multiple H1',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '2.4.6' ),
			'auto_fixable' => true,
		),
		'empty-heading'            => array(
			'name'         => 'Empty Heading',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '2.4.6' ),
			'auto_fixable' => true,
		),
		'skipped-heading-level'    => array(
			'name'         => 'Skipped Heading Level',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '1.3.1', '2.4.6' ),
			'auto_fixable' => true,
		),
		'heading-nesting'          => array(
			'name'         => 'Heading Nesting',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'heading-length'           => array(
			'name'         => 'Heading Length',
			'severity'     => self::SEVERITY_MINOR,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '2.4.6' ),
			'auto_fixable' => false,
		),
		'heading-uniqueness'       => array(
			'name'         => 'Heading Uniqueness',
			'severity'     => self::SEVERITY_MINOR,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '2.4.6' ),
			'auto_fixable' => false,
		),
		'heading-visual'           => array(
			'name'         => 'Heading Visual',
			'severity'     => self::SEVERITY_MINOR,
			'category'     => self::CAT_HEADINGS,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => false,
		),

		// ========== LINKS ==========
		'empty-link'               => array(
			'name'         => 'Empty Link',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.4', '4.1.2' ),
			'auto_fixable' => true,
		),
		'generic-link-text'        => array(
			'name'         => 'Generic Link Text',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.4', '2.4.9' ),
			'auto_fixable' => true,
		),
		'new-window-link'          => array(
			'name'         => 'New Window Link',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '3.2.5' ),
			'auto_fixable' => true,
		),
		'download-link'            => array(
			'name'         => 'Download Link',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.4' ),
			'auto_fixable' => true,
		),
		'external-link'            => array(
			'name'         => 'External Link',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.4' ),
			'auto_fixable' => true,
		),
		'link-destination'         => array(
			'name'         => 'Link Destination',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.4' ),
			'auto_fixable' => false,
		),
		'skip-link'                => array(
			'name'         => 'Skip Link',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_LINKS,
			'wcag'         => array( '2.4.1' ),
			'auto_fixable' => true,
		),

		// ========== FORMS ==========
		'missing-form-label'       => array(
			'name'         => 'Missing Form Label',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '1.3.1', '3.3.2', '4.1.2' ),
			'auto_fixable' => true,
		),
		'fieldset-legend'          => array(
			'name'         => 'Fieldset Legend',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '1.3.1', '3.3.2' ),
			'auto_fixable' => true,
		),
		'required-attribute'       => array(
			'name'         => 'Required Attribute',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '3.3.2' ),
			'auto_fixable' => true,
		),
		'error-message'            => array(
			'name'         => 'Error Message',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '3.3.1', '3.3.3' ),
			'auto_fixable' => true,
		),
		'autocomplete-attribute'   => array(
			'name'         => 'Autocomplete Attribute',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '1.3.5' ),
			'auto_fixable' => true,
		),
		'input-type'               => array(
			'name'         => 'Input Type',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '1.3.5' ),
			'auto_fixable' => true,
		),
		'placeholder-label'        => array(
			'name'         => 'Placeholder Label',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '3.3.2' ),
			'auto_fixable' => true,
		),
		'custom-control'           => array(
			'name'         => 'Custom Control',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => false,
		),
		'button-label'             => array(
			'name'         => 'Button Label',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'orphaned-label'           => array(
			'name'         => 'Orphaned Label',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'form-aria'                => array(
			'name'         => 'Form ARIA',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_FORMS,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),

		// ========== TABLES ==========
		'table-header'             => array(
			'name'         => 'Table Header',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'table-caption'            => array(
			'name'         => 'Table Caption',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'complex-table'            => array(
			'name'         => 'Complex Table',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => false,
		),
		'layout-table'             => array(
			'name'         => 'Layout Table',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'empty-table-cell'         => array(
			'name'         => 'Empty Table Cell',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_TABLES,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),

		// ========== MEDIA ==========
		'iframe-title'             => array(
			'name'         => 'Iframe Title',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_MEDIA,
			'wcag'         => array( '2.4.1', '4.1.2' ),
			'auto_fixable' => true,
		),
		'video-accessibility'      => array(
			'name'         => 'Video Accessibility',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_MEDIA,
			'wcag'         => array( '1.2.1', '1.2.2', '1.2.3' ),
			'auto_fixable' => false,
		),
		'audio-accessibility'      => array(
			'name'         => 'Audio Accessibility',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_MEDIA,
			'wcag'         => array( '1.2.1' ),
			'auto_fixable' => false,
		),
		'media-alternative'        => array(
			'name'         => 'Media Alternative',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_MEDIA,
			'wcag'         => array( '1.2.8' ),
			'auto_fixable' => false,
		),

		// ========== INTERACTIVITY ==========
		'positive-tabindex'        => array(
			'name'         => 'Positive Tabindex',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.4.3' ),
			'auto_fixable' => true,
		),
		'interactive-element'      => array(
			'name'         => 'Interactive Element',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.1.1', '4.1.2' ),
			'auto_fixable' => true,
		),
		'modal-accessibility'      => array(
			'name'         => 'Modal Accessibility',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.1.2', '2.4.3' ),
			'auto_fixable' => true,
		),
		'focus-indicator'          => array(
			'name'         => 'Focus Indicator',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.4.7' ),
			'auto_fixable' => true,
		),
		'keyboard-trap'            => array(
			'name'         => 'Keyboard Trap',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.1.2' ),
			'auto_fixable' => true,
		),
		'focus-order'              => array(
			'name'         => 'Focus Order',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_INTERACTIVITY,
			'wcag'         => array( '2.4.3' ),
			'auto_fixable' => false,
		),

		// ========== COLOR/CONTRAST ==========
		'text-color-contrast'      => array(
			'name'         => 'Text Color Contrast',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_COLOR_CONTRAST,
			'wcag'         => array( '1.4.3', '1.4.6' ),
			'auto_fixable' => false,
		),
		'color-reliance'           => array(
			'name'         => 'Color Reliance',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_COLOR_CONTRAST,
			'wcag'         => array( '1.4.1' ),
			'auto_fixable' => false,
		),
		'complex-contrast'         => array(
			'name'         => 'Complex Contrast',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_COLOR_CONTRAST,
			'wcag'         => array( '1.4.3' ),
			'auto_fixable' => false,
		),

		// ========== TOUCH/VIEWPORT ==========
		'touch-target'             => array(
			'name'         => 'Touch Target',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_TOUCH_VIEWPORT,
			'wcag'         => array( '2.5.5' ),
			'auto_fixable' => false,
		),
		'touch-gesture'            => array(
			'name'         => 'Touch Gesture',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_TOUCH_VIEWPORT,
			'wcag'         => array( '2.5.1' ),
			'auto_fixable' => false,
		),
		'viewport-check'           => array(
			'name'         => 'Viewport Check',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_TOUCH_VIEWPORT,
			'wcag'         => array( '1.4.10' ),
			'auto_fixable' => true,
		),

		// ========== ARIA ==========
		'aria-role'                => array(
			'name'         => 'ARIA Role',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'aria-attribute'           => array(
			'name'         => 'ARIA Attribute',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'aria-state'               => array(
			'name'         => 'ARIA State',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'landmark-role'            => array(
			'name'         => 'Landmark Role',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '1.3.1', '2.4.1' ),
			'auto_fixable' => true,
		),
		'redundant-aria'           => array(
			'name'         => 'Redundant ARIA',
			'severity'     => self::SEVERITY_MINOR,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'invalid-aria-combination' => array(
			'name'         => 'Invalid ARIA Combination',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => true,
		),
		'hidden-content'           => array(
			'name'         => 'Hidden Content',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_ARIA,
			'wcag'         => array( '4.1.2' ),
			'auto_fixable' => false,
		),

		// ========== STRUCTURE/CONTENT ==========
		'semantic-html'            => array(
			'name'         => 'Semantic HTML',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_STRUCTURE,
			'wcag'         => array( '1.3.1' ),
			'auto_fixable' => true,
		),
		'live-region'              => array(
			'name'         => 'Live Region',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_STRUCTURE,
			'wcag'         => array( '4.1.3' ),
			'auto_fixable' => true,
		),
		'page-structure'           => array(
			'name'         => 'Page Structure',
			'severity'     => self::SEVERITY_MODERATE,
			'category'     => self::CAT_STRUCTURE,
			'wcag'         => array( '1.3.1', '2.4.1' ),
			'auto_fixable' => false,
		),
		'language-change'          => array(
			'name'         => 'Language Change',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '3.1.2' ),
			'auto_fixable' => true,
		),
		'status-message'           => array(
			'name'         => 'Status Message',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '4.1.3' ),
			'auto_fixable' => true,
		),
		'error-identification'     => array(
			'name'         => 'Error Identification',
			'severity'     => self::SEVERITY_CRITICAL,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '3.3.1' ),
			'auto_fixable' => true,
		),
		'animation-pause'          => array(
			'name'         => 'Animation Pause',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '2.2.2' ),
			'auto_fixable' => true,
		),
		'timing-control'           => array(
			'name'         => 'Timing Control',
			'severity'     => self::SEVERITY_SERIOUS,
			'category'     => self::CAT_CONTENT,
			'wcag'         => array( '2.2.1' ),
			'auto_fixable' => true,
		),
	);

	/**
	 * Legacy/alias map -> canonical ID
	 * Normalizes old IDs and variations (underscores, "missing-" prefixes, etc.).
	 */
	private static $aliases = array(
		'generic-link'               => 'generic-link-text',
		'missing-label'              => 'missing-form-label',
		'redundant-alt'              => 'redundant-alt-text',
		'link-dest'                  => 'link-destination',
		'new-window'                 => 'new-window-link',
		'link-opens-new-window'      => 'new-window-link',
		'image-map'                  => 'image-map-alt',
		'missing-image-map-alt'      => 'image-map-alt',
		'alt-quality'                => 'alt-text-quality',
		'svg-access'                 => 'svg-accessibility',
		'missing-svg-title'          => 'svg-accessibility',
		'bg-image'                   => 'background-image',
		'heading-unique'             => 'heading-uniqueness',
		'contrast'                   => 'text-color-contrast',
		'color_contrast'             => 'text-color-contrast',
		'autocomplete'               => 'autocomplete-attribute',
		'required-attr'              => 'required-attribute',
		'missing-required-attribute' => 'required-attribute',
		'skipped-heading'            => 'skipped-heading-level',
		'modal-access'               => 'modal-accessibility',
		'widget-keyboard'            => 'interactive-element',
		'aria-attr'                  => 'aria-attribute',
		'missing-aria-label'         => 'aria-attribute',
		'invalid-aria'               => 'invalid-aria-combination',
		'media-alt'                  => 'media-alternative',
		'video-access'               => 'video-accessibility',
		'missing-video-caption'      => 'video-accessibility',
		'audio-access'               => 'audio-accessibility',
		'audio_accessibility'        => 'audio-accessibility',
		'empty-cell'                 => 'empty-table-cell',
		'missing-table-scope'        => 'table-scope',
		'viewport'                   => 'viewport-check',
		'improper-viewport'          => 'viewport-check',
		'invalid-tabindex'           => 'positive-tabindex',
		'missing-focus-indicator'    => 'focus-indicator',
		'missing-iframe-title'       => 'iframe-title',
		'missing-lang-attribute'     => 'lang-attribute',
		'missing-page-title'         => 'page-title',
		'missing-button-type'        => 'button-type',
		'missing-figure-caption'     => 'figure-caption',
		'improper-list-structure'    => 'list-structure',
		'missing-fieldset-legend'    => 'fieldset-legend',
		'missing-error-description'  => 'error-message',
		'missing-skip-link'          => 'skip-link',
	);

	/**
	 * Get all canonical IDs
	 *
	 * @return array
	 */
	public static function get_all(): array {
		return array_keys( self::$ids );
	}

	/**
	 * Get metadata for a canonical ID
	 *
	 * @param string $id
	 * @return array|null
	 */
	public static function get_metadata( string $id ): ?array {
		$canonical = self::canonicalize( $id );
		return $canonical ? self::$ids[ $canonical ] : null;
	}

	/**
	 * Backwards-compatible getter used by some tools/CLI commands.
	 *
	 * @param string $id
	 * @return array|null
	 */
	public static function get( string $id ): ?array {
		return self::get_metadata( $id );
	}

	/**
	 * Check if ID is valid
	 *
	 * @param string $id
	 * @return bool
	 */
	public static function is_valid( string $id ): bool {
		return self::canonicalize( $id ) !== null;
	}

	/**
	 * Normalize to canonical ID (hyphenated, alias-resolved)
	 */
	public static function canonicalize( string $id ): ?string {
		$normalized = strtolower( str_replace( '_', '-', trim( $id ) ) );

		if ( isset( self::$aliases[ $normalized ] ) ) {
			$normalized = self::$aliases[ $normalized ];
		}

		if ( isset( self::$ids[ $normalized ] ) ) {
			return $normalized;
		}

		return null;
	}

	/**
	 * Get IDs by category
	 *
	 * @param string $category
	 * @return array
	 */
	public static function get_by_category( string $category ): array {
		$result = array();
		foreach ( self::$ids as $id => $meta ) {
			if ( $meta['category'] === $category ) {
				$result[] = $id;
			}
		}
		return $result;
	}

	/**
	 * Get IDs by severity
	 *
	 * @param string $severity
	 * @return array
	 */
	public static function get_by_severity( string $severity ): array {
		$result = array();
		foreach ( self::$ids as $id => $meta ) {
			if ( $meta['severity'] === $severity ) {
				$result[] = $id;
			}
		}
		return $result;
	}

	/**
	 * Get auto-fixable IDs
	 *
	 * @return array
	 */
	public static function get_auto_fixable(): array {
		$result = array();
		foreach ( self::$ids as $id => $meta ) {
			if ( $meta['auto_fixable'] ) {
				$result[] = $id;
			}
		}
		return $result;
	}

	/**
	 * Get all categories
	 *
	 * @return array
	 */
	public static function get_categories(): array {
		return array(
			self::CAT_IMAGES         => 'Images',
			self::CAT_HEADINGS       => 'Headings',
			self::CAT_LINKS          => 'Links',
			self::CAT_FORMS          => 'Forms',
			self::CAT_TABLES         => 'Tables',
			self::CAT_MEDIA          => 'Media',
			self::CAT_INTERACTIVITY  => 'Interactivity',
			self::CAT_COLOR_CONTRAST => 'Color & Contrast',
			self::CAT_TOUCH_VIEWPORT => 'Touch & Viewport',
			self::CAT_ARIA           => 'ARIA',
			self::CAT_STRUCTURE      => 'Structure',
			self::CAT_CONTENT        => 'Content',
		);
	}

	/**
	 * Get count
	 *
	 * @return int
	 */
	public static function count(): int {
		return count( self::$ids );
	}
}
