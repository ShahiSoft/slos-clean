...
class Settings {
	...
	public function render() {
		...
		// Load template
		include SHAHI_LEGALFLOWSUITE_PATH . 'templates/admin/settings.php';
	}
	...
	public function render_banner_settings() {
		...
		include SHAHI_LEGALFLOWSUITE_PATH . 'templates/admin/banner-settings.php';
	}
	...
	private function get_tabs() {
		return array(
			'general'       => array(
				'title' => __( 'General', 'shahi-legalflowsuite' ),
				'icon'  => 'dashicons-admin-generic',
			),
			'notifications' => array(
				'title' => __( 'Notifications', 'shahi-legalflowsuite' ),
				'icon'  => 'dashicons-email',
			),
			'performance'   => array(
				'title' => __( 'Performance', 'shahi-legalflowsuite' ),
			...
			'uninstall'     => array(
				'title' => __( 'Uninstall', 'shahi-legalflowsuite' ),
	...
	private function get_default_settings() {
		return array(
			// General settings
			'plugin_name'                    => __( 'ShahiLegalFlowSuite', 'shahi-legalflowsuite' ),
			'enable_debug'                   => false,
			...
			'consent_banner_heading'         => __( 'We value your privacy', 'shahi-legalflowsuite' ),
			'consent_banner_message'         => __( 'We use cookies to enhance your browsing experience, serve personalized content, and analyze our traffic.', 'shahi-legalflowsuite' ),
			'consent_accept_button_text'     => __( 'Accept All', 'shahi-legalflowsuite' ),
			'consent_reject_button_text'     => __( 'Reject All', 'shahi-legalflowsuite' ),
			'consent_settings_button_text'   => __( 'Cookie Settings', 'shahi-legalflowsuite' ),
			'consent_privacy_policy_text'    => __( 'Privacy Policy', 'shahi-legalflowsuite' ),
			'privacy_policy_url'             => '',
			'learn_more_url'                 => '',
	...
	private function save_settings() {
		...
		if ( $updated ) {
			...
			add_settings_error(
				'shahi_settings',
				'settings_saved',
				__( 'Settings saved successfully.', 'shahi-legalflowsuite' ),
				'success'
			);
		} else {
			add_settings_error(
				'shahi_settings',
				'settings_unchanged',
				__( 'Settings unchanged or failed to save.', 'shahi-legalflowsuite' ),
				'info'
			);
		}
	}
	...
	public function ajax_restart_onboarding() {
		...
		if ( $still_completed ) {
			wp_send_json_error(
				array(
					'message' => __( 'Failed to reset onboarding wizard. Please try clearing your cache.', 'shahi-legalflowsuite' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message'  => __( 'Onboarding wizard has been reset successfully.', 'shahi-legalflowsuite' ),
