...
(function ($) {
	...
	const nonce =
		config.nonce || (window.slosScanner && window.slosScanner.nonce) || '';
	...
	window.SLOSScanProgress = {
		...
		cacheElements() {
			...
			this.elements.progressBar = $overlay.find(
				'.slos-scan-progress-bar'
			);
			this.elements.progressWrap = $overlay.find(
				'.slos-scan-progress-wrap'
			);
			this.elements.progressPercent = $overlay.find(
				'.slos-scan-progress-percent'
			);
			this.elements.progressStatus = $overlay.find(
				'.slos-scan-progress-status'
			);
			this.elements.pageList = $overlay.find('.slos-scan-page-list');
			this.elements.summaryScanned = $overlay.find(
				'[data-stat="scanned"]'
			);
			this.elements.summaryIssues = $overlay.find('[data-stat="issues"]');
			this.elements.summaryCritical = $overlay.find(
				'[data-stat="critical"]'
			);
			this.elements.summaryClean = $overlay.find('[data-stat="clean"]');
			this.elements.cancelBtn = $overlay.find('.slos-scan-btn-cancel');
			this.elements.closeActionBtn = $overlay.find(
				'.slos-scan-close-action'
			);
			this.elements.viewResultsBtn = $overlay.find(
				'.slos-scan-view-results'
			);
		},
		...
		bindEvents() {
			...
			$(document).on('click', '.slos-scan-overlay', function (e) {
				if (
					$(e.target).hasClass('slos-scan-overlay') &&
		...
		start(options = {}) {
			...
			$.ajax({
				...
				success(response) {
					...
					self.announce(
						'Scanning ' + self.state.totalPages + ' pages'
					);
				...
				error() {
					self.updateProgress(
						0,
						'Error fetching pages. Please try again.'
					);
					self.complete();
				},
			});
		...
		scanPages() {
			...
			function processNext() {
				while (
					activeCount < self.config.batchSize &&
					queue.length > 0
				) {
					...
					self.scanPage(page, function () {
						activeCount--;
						if (
							self.state.scannedPages >= self.state.totalPages ||
			...
			const xhr = $.ajax({
				...
				complete() {
					self.state.activeRequests =
						self.state.activeRequests.filter(function (r) {
							return r !== xhr;
						});
		...
		finishScan() {
			...
			if (this.state.isCancelled) {
				this.updateProgress(
					Math.round(
						(this.state.scannedPages / this.state.totalPages) * 100
					),
					'Scan cancelled. Scanned ' +
			...
			$.ajax({
				...
				complete() {
					const cleanPages =
						self.state.totalPages - self.state.pagesWithIssues;
					self.updateProgress(
						100,
						'Scan complete! Found ' +
							self.state.totalIssues +
							' issues across ' +
							self.state.pagesWithIssues +
							' pages.'
					);
		...
		renderPageList() {
			...
			this.state.pages
				.slice(0, this.config.maxVisiblePages)
		...
		createPageItem(page) {
			...
			const item = $('<div>', {
				...
				'<div class="slos-scan-page-status ' +
					page.status +
					'">' +
					statusIcon +
					'</div>' +
					'<div class="slos-scan-page-info">' +
					'<div class="slos-scan-page-title">' +
					this.escapeHtml(page.title) +
					'</div>' +
					'</div>' +
					'<div class="slos-scan-page-issues">' +
					issuesBadge +
					criticalBadge +
					'</div>'
			);
		 ...
		 */
		updatePageItem(page) {
			const $item = this.elements.pageList.find(
				'[data-page-id="' + page.id + '"]'
			);
		...
		updateSummary() {
			const cleanPages =
				this.state.scannedPages - this.state.pagesWithIssues;
		...
		cancel() {
			const confirmMsg =
				(config.i18n && config.i18n.confirmCancel) ||
