...
(function () {
	...
	function matchesPattern(url, pattern) {
		...
		if (pattern.startsWith( '/' ) && pattern.includes( '/' )) {
			try {
				...
				console.warn(
					'[Consent Blocker] Invalid regex pattern:',
					pattern
				);
			}
		}
	...
	XMLHttpRequest.prototype.send = function () {
		if (this._consentBlocked) {
			console.log(
				`[Consent Blocker] Prevented XHR send for rule: ${this._blockingRule.id}`
			);
	...
	window.complyflowReplayScripts = function (updatedConsent) {
		...
		console.log(
			`[Consent Blocker] Replaying ${blockedScripts.length} blocked scripts`
		);
