	...
	const SLOSDocGenerate = {
		...
		showMissingFieldsError: function(missingFields) {
			...
			missingFields.forEach(field => {
				...
				html += '<a href="' + profileUrl + '" class="button button-small button-primary">Complete Now →</a>';
				html += '</li>';
			});
		...
		renderField: function(field, missingFields) {
			...
			if (hasValue) {
				...
				$actions.append(
					$('<button type="button" class="slos-gen-field__btn slos-gen-field__btn--edit" title="Edit value">').html('✏️ <span>Edit</span>'),
					$('<button type="button" class="slos-gen-field__btn slos-gen-field__btn--remove" title="Exclude field">').html('🗑️')
				);
			} else if (!isMandatory) {
				// Add button for optional empty fields
				$actions.append(
					$('<button type="button" class="slos-gen-field__btn slos-gen-field__btn--add" title="Add value">').html('➕ <span>Add</span>')
				);
			}
			
			// Save/Cancel buttons (hidden by default, shown during editing)
			$actions.append(
				$('<button type="button" class="slos-gen-field__btn slos-gen-field__btn--save" title="Save changes" style="display:none;">').html('✓ <span>Save</span>'),
				$('<button type="button" class="slos-gen-field__btn slos-gen-field__btn--cancel" title="Cancel editing" style="display:none;">').html('✕ <span>Cancel</span>')
			);
		...
		handlePreview: function(e) {
			...
			$btn.prop('disabled', true).text(SLOSDocGen.i18n.loading || '⏳ Generating Preview...');
		...
		handleGenerateDocument: function(e) {
			...
			$btn.addClass('submitting').text(SLOSDocGen.i18n.generating || '⏳ Generating...');
		...
		handleRegenerateAll: function(e) {
			...
			$btn.prop('disabled', true).html('<span class="slos-gen-action-link__icon">⏳</span><span class="slos-gen-action-link__text">Regenerating...</span>');
		...
		handleExportAll: function(e) {
			...
			const $btn = $(e.currentTarget);
			const originalHtml = $btn.html();
			$btn.prop('disabled', true).html('<span class="slos-gen-action-link__icon">⏳</span><span class="slos-gen-action-link__text">Exporting...</span>');
		...
		handleClearDrafts: function(e) {
			...
			const originalHtml = $btn.html();
			$btn.prop('disabled', true).html('<span class="slos-gen-action-link__icon">⏳</span><span class="slos-gen-action-link__text">Clearing...</span>');
				...
				const $item = $(`
					<div class="slos-gen-timeline-item ${isCurrent ? 'slos-gen-timeline-item--current' : ''}">
						...
						<div class="slos-gen-timeline-item__meta">
							...
							${version.author ? `<span class="slos-gen-timeline-item__meta-item">👤 ${version.author}</span>` : ''}
						</div>
			...
			let html = `
				<div class="slos-gen-comparison-header">
					<p class="slos-gen-comparison-dates">
						...
						<strong>${data.version2_at || 'Unknown'}</strong> → <strong>${data.version1_at || 'Unknown'}</strong>
					</p>
				</div>
