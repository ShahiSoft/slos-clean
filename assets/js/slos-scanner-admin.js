jQuery(document).ready(function ($) {
	...
	console.log(
		'SLOSAutoFixProgress available:',
		typeof window.SLOSAutoFixProgress
	);
	console.log(
		'slosautoFixConfig available:',
		typeof window.slosautoFixConfig
	);
	console.log('slosScanner available:', typeof window.slosScanner);
	console.log(
		'.slos-fix-all-btn buttons found:',
		$('.slos-fix-all-btn').length
	);
	...
	$('#slos-start-scan').on('click', function (e) {
		...
		if (
			typeof window.SLOSScanProgress !== 'undefined' &&
			typeof window.SLOSScanProgress.start === 'function'
		) {
			// Initialize modal once if not already in DOM
			if (
				typeof window.SLOSScanProgress.init === 'function' &&
				!$('#slos-scan-progress-overlay').length
			) {
	...
	function updateResultsPreview() {
		$('#slos-scan-results').html(
			'<p>Found issues in ' + results.length + ' posts so far...</p>'
		);
	}
	...
	function renderFinalResults() {
		if (results.length === 0) {
			$('#slos-scan-results').html(
				'<div class="notice notice-success inline"><p>Great job! No accessibility issues found.</p></div>'
			);
			return;
		}

		let html =
			'<div class="slos-actions-bar" style="margin-bottom: 15px;">';
		html +=
			'<button id="slos-export-csv" class="button">Export CSV Report</button>';
		html += '</div>';

		html += '<table class="wp-list-table widefat fixed striped">';
		html +=
			'<thead><tr><th>Post Title</th><th>Issues Found</th><th>Actions</th></tr></thead><tbody>';
		...
		results.forEach(function (item) {
			...
			html +=
				...
				'</a></td>';
			html +=
				'<td><span class="slos-issue-count">' +
				item.issues_count +
				' Issues</span></td>';
			html += '<td>';
			html +=
				'<a href="' +
				item.edit_link +
				'" class="button button-small">Edit & Fix</a> ';
	...
	$(document).on('click', '#slos-publish-statement', function () {
		...
		if (
			!confirm(
				'This will create/update a page titled "Accessibility Statement". Continue?'
			)
		) {
			return;
		}
		...
		const originalText = $btn.html();
		$btn.prop('disabled', true).html(
			'<span class="dashicons dashicons-update spin"></span> Publishing...'
		);
		...
		$.ajax({
			...
			success(response) {
				...
				if (response.success) {
					...
					const notification =
						'<div class="notice notice-success inline" style="margin-top: 16px; padding: 12px; background: rgba(34, 197, 94, 0.1); border-left: 3px solid var(--slos-success); border-radius: 6px;">' +
						'<p style="margin: 0; color: var(--slos-text-primary);">' +
						'<strong>Statement Published Successfully!</strong><br>' +
						'<a href="' +
						editLink +
						'" target="_blank" style="color: var(--slos-accent); text-decoration: none; margin-right: 12px;">Edit Page</a> | ' +
						'<a href="' +
						viewLink +
						'" target="_blank" style="color: var(--slos-accent); text-decoration: none;">View Page</a>' +
						'</p></div>';
					...
					if ($('#slos-statement-notification').length) {
						...
						$('#slos-statement-preview-anchor').before(
							'<div id="slos-statement-notification">' +
								notification +
								'</div>'
						);
					}
					...
					alert(
						'Error: ' +
							(response.data || 'Failed to publish statement')
					);
				}
			},
			error(xhr, status, error) {
	...
	function showStatementPreview(html, raw) {
		...
		$('.slos-copy-statement')
			...
			.on('click', function () {
				const $btn = $(this);
				const textToCopy =
					raw || $('#slos-statement-preview-content').text();
				...
				navigator.clipboard
					...
					.then(function () {
						...
						$btn.html(
							'<span class="dashicons dashicons-yes" style="font-size: 14px;"></span> Copied!'
						);
						setTimeout(function () {
	...
	function showReportNotification(type, message) {
		...
		if (!$status.length) {
			const $container = $('#slos-schedule-report').closest(
				'.slos-tool-content'
			);
	...
	function handleDownloadButton($btn, action, extraFields) {
		const originalHtml = $btn.html();
		$btn.prop('disabled', true).html(
			'<span class="dashicons dashicons-update spin"></span> Preparing...'
		);
	...
	$(document).on('click', '.slos-export-pdf', function (e) {
		...
		handleDownloadButton($btn, 'slos_export_pdf', { template });
	});
	...
	$(document).on('click', '#slos-schedule-report', function (e) {
		...
		const originalHtml = $btn.html();
		$btn.prop('disabled', true).html(
			'<span class="dashicons dashicons-update spin"></span> Scheduling...'
		);
		...
		$.ajax({
			...
			error() {
				...
				showReportNotification(
					'error',
					'Network error while scheduling report.'
				);
			},
		});
	});
	...
	$(document).on('click', '#slos-send-test-report', function (e) {
		...
		if (!email || !isValidEmail(email)) {
			showReportNotification(
				'error',
				'Please enter a valid email address to send the test report.'
			);
			return;
		}
		...
		const originalHtml = $btn.html();
		$btn.prop('disabled', true).html(
			'<span class="dashicons dashicons-email-alt"></span> Sending...'
		);
		...
		$.ajax({
			...
			error() {
				...
				showReportNotification(
					'error',
					'Network error while sending test report.'
				);
			},
		});
	});
	...
	function initAutoFixHandlers() {
		...
		if (
			typeof window.slosautoFixConfig !== 'undefined' &&
			window.slosautoFixConfig.dormant === true
		) {
			console.log(
				'SLOS: Autofix is dormant, skipping autofix handler initialization'
			);
			return;
		}
		...
		$(document).on(
			'click',
			'.slos-autofix-trigger, #slos-autofix-all-btn, .slos-fix-all-btn, .slos-autofix-post-btn',
			function (e) {
				...
				const pageId =
					$btn.data('page-id') || $btn.data('post-id') || 0;
				...
				if (typeof window.SLOSAutoFixProgress === 'undefined') {
					...
					alert(
						'Auto-fix progress module not loaded. Please refresh the page.'
					);
					return;
				}
					...
					onComplete(results) {
						...
						if (results.fixed > 0) {
							...
							if ($row.length) {
								...
								const $issuesSpan = $row.find(
									'.slos-page-issues, .slos-issue-count'
								);
								if ($issuesSpan.length) {
									const currentCount =
										parseInt($issuesSpan.text()) || 0;
									const newCount = Math.max(
										0,
										currentCount - results.fixed
									);
							...
							$btn.html(
								'<span class="dashicons dashicons-yes"></span> No Issues'
							);
						}
						...
						if (results.fixed > 0) {
							...
							setTimeout(function () {
								alert(
									'No fixable issues found on this page.\n\n' +
	...
	function initRollbackHandlers() {
		...
		$(document).on('click', '.slos-rollback-btn', function (e) {
			...
			if (
				!confirm(
					'Are you sure you want to rollback accessibility fixes for this page?\n\nThis will restore the previous content from backup.'
				)
			) {
				return;
			}
			...
			$.ajax({
				...
				data: {
					...
					nonce:
						typeof window.slosautoFixConfig !== 'undefined'
							? window.slosautoFixConfig.nonce
							: slosScanner.nonce,
					post_id: postId,
				},
				success(response) {
	...
	$(document).on('click', '#slos-check-contrast', function () {
		...
		if (!fgColor || !bgColor) {
			$result
				.html(
					'<div style="color: var(--slos-error);">⚠️ Enter both colors</div>'
				)
				.addClass('show fail')
				.removeClass('pass');
			return;
		...
		if (!hexPattern.test(fgColor) || !hexPattern.test(bgColor)) {
			$result
				.html(
					'<div style="color: var(--slos-error);">⚠️ Invalid color format. Use #RRGGBB</div>'
				)
				.addClass('show fail')
				.removeClass('pass');
			return;
		...
		$.ajax({
			...
			success(response) {
				...
				if (response.success && response.data) {
					...
					let html =
						'<div style="display: flex; align-items: center; gap: 16px; padding: 16px; background: var(--slos-bg-card); border-radius: 6px;">';
					html += '<div style="display: flex; gap: 8px;">';
					html +=
						'<div style="width: 40px; height: 40px; background: ' +
						fgColor +
						'; border: 1px solid var(--slos-border); border-radius: 4px;"></div>';
					html +=
						'<div style="width: 40px; height: 40px; background: ' +
						bgColor +
						'; border: 1px solid var(--slos-border); border-radius: 4px;"></div>';
					...
					html +=
						'<div style="font-size: 18px; font-weight: 700; color: var(--slos-text-primary); margin-bottom: 4px;">Ratio: ' +
						d.ratio +
						':1</div>';
					html +=
						'<div style="font-size: 13px; color: var(--slos-text-muted);">' +
						d.recommendation +
						'</div>';
					html +=
					...
					html +=
						...
						(d.wcag_aa_normal ? 'pass' : 'fail') +
						'">AA Normal: ' +
						(d.wcag_aa_normal ? '✓' : '✗') +
						'</span>';
					html +=
						'<span class="wcag-badge ' +
						(d.wcag_aa_large ? 'pass' : 'fail') +
						'">AA Large: ' +
						(d.wcag_aa_large ? '✓' : '✗') +
						'</span>';
					html +=
						'<span class="wcag-badge ' +
						(d.wcag_aaa_normal ? 'pass' : 'fail') +
						'">AAA Normal: ' +
						(d.wcag_aaa_normal ? '✓' : '✗') +
						'</span>';
					html += '</div></div></div>';
					$result
						.html(html)
						.addClass('show')
						.removeClass('fail pass')
						.addClass(d.passes_aa ? 'pass' : 'fail');
				} else {
					$result
						.html(
							'<div style="color: var(--slos-error);">Error: ' +
			...
			error() {
				...
				$result
					.html(
						'<div style="color: var(--slos-error);">Network error</div>'
					)
					.addClass('show fail')
					.removeClass('pass');
			},
	...
	$(document).on('click', '#slos-check-readability', function () {
		...
		if (!text) {
			$result
				.html(
					'<div style="color: var(--slos-error);">⚠️ Please enter text to analyze</div>'
				)
				.addClass('show fail')
				.removeClass('pass');
			return;
		...
		if (text.split(/\s+/).length < 10) {
			$result
				.html(
					'<div style="color: var(--slos-error);">⚠️ Please enter at least 10 words for accurate analysis</div>'
				)
				.addClass('show fail')
				.removeClass('pass');
			return;
		...
		$.ajax({
			...
			success(response) {
				...
				if (response.success && response.data) {
					const d = response.data;
					let html =
						'<div style="padding: 16px; background: var(--slos-bg-card); border-radius: 6px;">';
					html +=
						'<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">';
					html +=
						'<div style="width: 60px; height: 60px; background: var(--slos-accent); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 700; color: white;">' +
						d.grade_level +
						'</div>';
					html += '<div style="flex: 1;">';
					html +=
						'<div style="font-size: 16px; font-weight: 700; color: var(--slos-text-primary); margin-bottom: 4px;">Grade Level: ' +
						d.grade_level +
						'</div>';
					html +=
						'<div style="font-size: 13px; color: var(--slos-text-muted);">' +
					...
					html += '</div></div>';
					html +=
						'<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; padding: 12px; background: var(--slos-bg-primary); border-radius: 4px;">';
					html +=
						'<div style="text-align: center;"><div style="font-size: 11px; color: var(--slos-text-muted); margin-bottom: 4px;">Words</div><div style="font-size: 18px; font-weight: 600; color: var(--slos-text-primary);">' +
						d.word_count +
						'</div></div>';
					html +=
						'<div style="text-align: center;"><div style="font-size: 11px; color: var(--slos-text-muted); margin-bottom: 4px;">Sentences</div><div style="font-size: 18px; font-weight: 600; color: var(--slos-text-primary);">' +
						d.sentence_count +
						'</div></div>';
					html +=
						'<div style="text-align: center;"><div style="font-size: 11px; color: var(--slos-text-muted); margin-bottom: 4px;">Syllables</div><div style="font-size: 18px; font-weight: 600; color: var(--slos-text-primary);">' +
						d.syllable_count +
						'</div></div>';
					html +=
						'<div style="text-align: center;"><div style="font-size: 11px; color: var(--slos-text-muted); margin-bottom: 4px;">Reading Ease</div><div style="font-size: 18px; font-weight: 600; color: var(--slos-text-primary);">' +
						d.flesch_score.toFixed(1) +
						'</div></div>';
					html += '</div></div>';
					...
					$result
						...
						.addClass('show')
						.removeClass('fail pass')
						.addClass(passesReadability ? 'pass' : 'fail');
				} else {
					$result
						.html(
							'<div style="color: var(--slos-error);">Error: ' +
	...
	$(document).on('click', '#slos-check-link', function () {
		...
		if (!linkText) {
			$result
				.html(
					'<div style="color: var(--slos-error);">⚠️ Please enter link text to validate</div>'
				)
				.addClass('show fail')
				.removeClass('pass');
			return;
			...
			success(response) {
				...
				if (response.success && response.data) {
					...
					if (d.is_descriptive) {
						...
						html +=
							'<div style="width: 40px; height: 40px; background: var(--slos-success); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px; color: white;">✓</div>';
						html += '<div style="flex: 1;">';
						html +=
							'<div style="font-size: 16px; font-weight: 700; color: var(--slos-success); margin-bottom: 4px;">Descriptive Link Text</div>';
						html +=
							'<div style="font-size: 13px; color: var(--slos-text-muted);">This link text appears to be descriptive and accessible.</div>';
						html += '</div></div>';
						html +=
							'<div style="padding: 12px; background: var(--slos-bg-primary); border-radius: 4px; border-left: 3px solid var(--slos-success);">';
						html +=
							'<div style="font-size: 12px; font-weight: 600; color: var(--slos-text-primary); margin-bottom: 4px;">✓ WCAG 2.4.4 Compliant</div>';
						html +=
							'<div style="font-size: 11px; color: var(--slos-text-muted);">Link purpose can be determined from the link text alone.</div>';
						html += '</div>';
					} else {
						html +=
							'<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">';
						html +=
							'<div style="width: 40px; height: 40px; background: var(--slos-error); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px; color: white;">✗</div>';
						html += '<div style="flex: 1;">';
						html +=
							'<div style="font-size: 16px; font-weight: 700; color: var(--slos-error); margin-bottom: 4px;">Generic Link Text</div>';
						html +=
							'<div style="font-size: 13px; color: var(--slos-text-muted);">This link text is not descriptive enough.</div>';
						html += '</div></div>';

						if (d.pattern_match) {
							html +=
								'<div style="padding: 12px; background: var(--slos-bg-primary); border-radius: 4px; border-left: 3px solid var(--slos-error); margin-bottom: 12px;">';
							html +=
								'<div style="font-size: 12px; font-weight: 600; color: var(--slos-text-primary); margin-bottom: 4px;">✗ WCAG 2.4.4 Issue</div>';
							if (d.pattern_match === 'too_short') {
								html +=
									'<div style="font-size: 11px; color: var(--slos-text-muted);">Link text is too short to be descriptive.</div>';
							} else {
								html +=
									'<div style="font-size: 11px; color: var(--slos-text-muted);">Detected generic pattern: "' +
									d.pattern_match +
									'"</div>';
							}
							html += '</div>';
						...
						if (d.suggestions && d.suggestions.length > 0) {
							...
							html +=
								'<div style="font-size: 13px; font-weight: 600; color: var(--slos-text-primary); margin-bottom: 8px;">💡 Suggestions:</div>';
							html +=
								'<ul style="margin: 0; padding-left: 20px; font-size: 12px; color: var(--slos-text-muted); line-height: 1.6;">';
					...
					$result
						...
						.addClass('show')
						.removeClass('fail pass')
						.addClass(d.is_descriptive ? 'pass' : 'fail');
				} else {
					$result
						.html(
							'<div style="color: var(--slos-error);">Error: ' +
	...
	$(document).on('click', '#slos-save-config', function () {
		...
		$btn.prop('disabled', true).html(
			'<span class="dashicons dashicons-update slos-spin"></span> Saving...'
		);
		...
		$.ajax({
			...
			success(response) {
				...
				if (response.success) {
					// Show success notification
					const $notification = $(
						'<div class="slos-notification success">'
					)
						.html(
							'<span class="dashicons dashicons-yes-alt"></span> Configuration saved successfully!'
						)
						.css({
							position: 'fixed',
							top: '32px',
	...
	$(document).on('click', '#slos-reset-config', function () {
		if (
			!confirm(
				'Are you sure you want to reset all scanner configuration to defaults? This action cannot be undone.'
			)
		) {
			return;
		}
		...
		$(
			'.slos-scan-post-type[value="post"], .slos-scan-post-type[value="page"]'
		).prop('checked', true);
		...
		alert(
			'Configuration reset to defaults. Click "Save Configuration" to apply changes.'
		);
	});
	...
	$(document).on('click', '.slos-category-toggle-all', function () {
		...
		const $checkers = $(
			'.category-checkers[data-category="' +
				category +
				'"] .slos-checker-toggle'
		);
		const allChecked =
			$checkers.length === $checkers.filter(':checked').length;
	...
	function initWidgetPreview() {
		const $preview = $('#slos-widget-preview');
		const position =
			$('input[name="slos-widget-position"]:checked').val() ||
			'bottom-right';
		const color =
			$('input[name="slos-widget-color"]:checked').val() || 'blue';
		...
		$preview.removeClass(
			'theme-blue theme-green theme-purple theme-orange theme-red theme-teal'
		);
	...
	$(document).on('click', '#slos-save-widget-config', function (e) {
		...
		const enabled = $('#slos-widget-enabled').is(':checked');
		const position =
			$('input[name="slos-widget-position"]:checked').val() ||
			'bottom-right';
		const color =
		...
		$.ajax({
			...
			success(response) {
				if (response.success) {
					...
					$btn.html(
						'<span class="dashicons dashicons-yes"></span> Saved!'
					);
			...
			error(xhr, status, error) {
				...
				showReportNotification(
					'error',
					'An error occurred while saving widget configuration.'
				);
			},
		});
	});
	...
	$(document).on('click', '#slos-wcag-run-scan', function (e) {
		...
		$('html, body').animate(
			{
				scrollTop:
					$('#slos-start-scan').closest('.slos-tools-card').offset()
						.top - 100,
			},
			500
		);
	...
	$(document).on('click', '#slos-wcag-fix-all', function (e) {
		...
		if (
			!confirm(
				'This will attempt to automatically fix all accessibility issues across all pages. Continue?'
			)
		) {
			return;
		}
