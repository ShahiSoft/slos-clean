	...
	class ConsentBanner {
		...
		constructor() {
			...
			this.showDescriptions =
				this.config.showDescriptions !== undefined
			...
			this.bannerVersion =
				this.config.bannerVersion || this.generateBannerVersion();
		...
		async resolveRegionAndTemplate() {
			try {
				...
				if (this.config.matchingRule && this.config.matchingRule.id) {
					...
					console.log(
						'[SLOS] Using geo rule from backend:',
						this.config.matchingRule.name
					);
				...
				if (!this.region) {
					const geoUrl =
						(this.routes && this.routes.geo) ||
						(this.config.apiUrl
							? this.config.apiUrl.replace('/consents', '') +
							  '/geo/region'
							: '/wp-json/slos/v1/geo/region');
					...
					if (payload && payload.data && payload.data.region) {
						...
						if (!this.config.template && payload.data.template) {
							...
							console.log(
								'[SLOS] Using geo-suggested template:',
								this.bannerTemplate
							);
						}
					}
					...
					if (!this.config.template) {
						this.bannerTemplate = this.mapRegionToTemplate(
							this.region
						);
						console.log(
							'[SLOS] Using fallback template for region',
							this.region + ':',
							this.bannerTemplate
						);
					}
			...
			if (this.userId > 0) {
				try {
					const response = await fetch(
						`${this.apiUrl}/check?user_id=${this.userId}&type=${purpose}`
					);
					const data = await response.json();
		...
		getFromLocalStorage(purpose) {
			try {
				const consents = JSON.parse(
					localStorage.getItem('slos_consents') || '{}'
				);
				if (
					consents[purpose] &&
					consents[purpose].granted !== undefined
				) {
		...
		showBanner() {
			...
			document.dispatchEvent(
				new CustomEvent('slos-consent-shown', {
					detail: {
		...
		createBanner() {
			...
			div.setAttribute(
				'aria-label',
				this.t('bannerTitle', 'Consent Preferences')
			);
		 ...
		 */
		getEUBannerHTML() {
			const purposeOptions = this.purposes
				.map((purpose) => {
					const isRequired =
						purpose === 'necessary' || purpose === 'functional';
					const description = this.getCategoryDescription(purpose);
					const vendorList = this.showVendors
						? this.getVendorList(purpose)
						: '';
                    ...
                    <div class="slos-consent-option" data-category="${purpose}">
                        <div class="slos-consent-header">
                            <label class="slos-consent-label">
                                <span class="slos-purpose-label">${this.formatPurpose(
									purpose
								)}</span>
                                ${
									isRequired
										? '<span class="slos-required">' +
                                ...
                                <input type="checkbox" 
                                       ...
                                       aria-label="${this.formatPurpose(
											purpose
										)}">
                            </label>
                            ${
								(this.showDescriptions && description) ||
								vendorList
									? `<button class="slos-expand-toggle" data-purpose="${purpose}" aria-expanded="false" aria-label="Show details for ${this.formatPurpose(
											purpose
									  )}">
                        ...
                        ${
							(this.showDescriptions && description) || vendorList
                                ...
                                ${
									this.showDescriptions && description
										? `<p class="slos-category-description">${description}</p>`
										: ''
								}
                                ${vendorList}
                            </div>`
			...
			return `
                <div class="slos-banner-content" role="main">
                    <button class="slos-banner-close" data-action="dismiss" type="button" aria-label="${this.t(
						'closeBanner',
						'Close banner'
					)}">
                    ...
                    <div class="slos-banner-header">
                        <h2 class="slos-banner-title">${this.t(
							'euTitle',
                    ...
                    <div class="slos-banner-body">
                        <p class="slos-banner-message">
                            ${this.t(
								'euMessage',
								'We use cookies to enhance your experience. Click "Accept" to consent or customize your preferences.'
							)}
                            ${
								this.privacyUrl
									? ` <a href="${this.privacyUrl}" class="slos-privacy-link" target="_blank" rel="noopener noreferrer">${this.learnMoreText}</a>`
									: ''
							}
                        </p>
                    ...
                    <div class="slos-banner-footer">
                        <button class="slos-btn slos-btn-accept-all" data-action="accept-all" type="button">${this.t(
							'acceptAll',
							'Accept All'
						)}</button>
                        <button class="slos-btn slos-btn-reject-all" data-action="reject-all" type="button">${this.t(
							'rejectAll',
							'Reject'
						)}</button>
                        <a href="#" class="slos-settings-link" data-action="toggle-options">${this.t(
							'customize',
							'Customize'
						)}</a>
                    </div>
                ...
                <div class="slos-banner-content" role="main">
                    ...
                    <div class="slos-banner-body">
                        <p class="slos-banner-message">
                            ${this.t(
								'ccpaMessage',
								'We use cookies. CA residents have the right to opt-out.'
							)}
                            ${
								this.privacyUrl
									? ` <a href="${this.privacyUrl}" class="slos-privacy-link" target="_blank" rel="noopener noreferrer">${this.learnMoreText}</a>`
                    ...
                    <div class="slos-banner-footer">
                        ...
                        <button class="slos-btn slos-btn-reject-all" data-action="do-not-sell" type="button">${this.t(
							'doNotSell',
							'Do Not Sell'
						)}</button>
                    </div>
		...
		getSimpleBannerHTML() {
			return `
                <div class="slos-banner-content" role="main">
                    ...
                    <div class="slos-banner-footer">
                        <button class="slos-btn slos-btn-accept" data-action="accept-all" type="button">${this.t(
							'accept',
							'OK'
						)}</button>
                        <button class="slos-btn slos-btn-reject" data-action="reject-all" type="button">${this.t(
							'decline',
							'No'
						)}</button>
                    </div>
		...
		bindEvents() {
			...
			banner
				.querySelectorAll(
					'[data-action="reject-all"], [data-action="do-not-sell"]'
				)
				.forEach((btn) => {
					btn.addEventListener('click', (e) => {
						e.preventDefault();
			...
			// Accept selected
			banner
				.querySelectorAll('[data-action="accept-selected"]')
				.forEach((btn) => {
					btn.addEventListener('click', (e) => {
						e.preventDefault();
			...
			banner
				.querySelectorAll('[data-action="toggle-options"]')
				.forEach((link) => {
			...
			banner
				.querySelectorAll('.slos-consent-checkbox')
				.forEach((checkbox) => {
			...
			if (options) {
				...
				if (options.classList.contains('slos-expanded')) {
					...
					if (
						...
						footer.insertBefore(
							saveBtn,
							footer.querySelector('.slos-settings-link')
						);
					}
				} else {
		...
		showButtonLoading(button) {
			...
			button.innerHTML =
				'<span class="slos-spinner" style="display: inline-block; width: 14px; height: 14px; border: 2px solid currentColor; border-top-color: transparent; border-radius: 50%; animation: slos-spin 0.6s linear infinite; margin-right: 6px;"></span> Saving...';
		}
		...
		async acceptAll(event) {
			...
			try {
				...
				const acceptedCategories = this.purposes.filter(
					(p) => this.consents[p] === true
				);
				const startTime = performance.now();
				const duration = startTime; // Time from page load to consent

				document.dispatchEvent(
					new CustomEvent('slos-consent-accepted', {
						detail: {
							timestamp: new Date().toISOString(),
							action: 'accept_all',
		...
		async rejectAll(event) {
			...
			try {
				...
				const rejectedCategories = this.purposes.filter(
					(p) => this.consents[p] === false
				);
				const acceptedCategories = this.purposes.filter(
					(p) => this.consents[p] === true
				);
				...
				document.dispatchEvent(
					new CustomEvent('slos-consent-rejected', {
						detail: {
							timestamp: new Date().toISOString(),
							action: 'reject_all',
		...
		async acceptSelected(event) {
			...
			try {
				const checkboxes = document.querySelectorAll(
					'.slos-consent-checkbox:checked'
				);
				const selectedPurposes = Array.from(checkboxes).map(
					(cb) => cb.dataset.purpose
				);
				...
				document.dispatchEvent(
					new CustomEvent('slos-consent-customized', {
						detail: {
							timestamp: new Date().toISOString(),
							action: 'customize',
							categoriesAccepted: acceptedCategories,
		...
		async grantConsent(purpose) {
			try {
				...
				const consentText =
					bannerElement?.querySelector('p.slos-banner-message')
						?.textContent || 'Consent granted via banner';

				// Phase 3.2: Collect categories_accepted for audit logging
				const categoriesAccepted = this.purposes.filter(
					(p) => this.consents[p] === true
				);
		...
		saveConsentMetadata() {
			try {
				const metadata = {
					policyVersion: this.policyVersion,
					bannerVersion: this.bannerVersion,
					categoriesAccepted: this.purposes.filter(
						(p) => this.consents[p] === true
					),
					timestamp: Date.now(),
					template: this.bannerTemplate,
		...
		assignVariant() {
			...
			try {
				...
				console.warn(
					'[SLOS] Could not read variant from localStorage:',
					e
				);
			}
			...
			try {
				...
				console.warn(
					'[SLOS] Could not store variant in localStorage:',
					e
				);
			}
		...
		emitConsentSignals() {
			...
			document.dispatchEvent(
				new CustomEvent('slos-consent-updated', {
					detail: {
		...
		generateConsentReceipt() {
			...
			return {
				receiptId: `slos-receipt-${Date.now()}-${Math.random()
					.toString(36)
					.substr(2, 9)}`,
				timestamp,
		...
		downloadConsentReceipt(receipt) {
			try {
				...
				console.log(
					'[SLOS] Consent receipt downloaded:',
					receiptData.receiptId
				);
			} catch (error) {
				console.error(
					'[SLOS] Failed to download consent receipt:',
					error
				);
			}
		}
		...
		buildConsentModeState() {
			return {
				...
				analytics_storage: this.consents.analytics
					? 'granted'
					: 'denied',
				ad_user_data: this.consents.marketing ? 'granted' : 'denied',
				ad_personalization: this.consents.personalization
					? 'granted'
					: 'denied',
				functionality_storage: this.consents.functional
					? 'granted'
					: 'denied',
				personalization_storage: this.consents.preferences
					? 'granted'
					: 'denied',
			};
		}
		...
		reloadScripts() {
			...
			document.dispatchEvent(
				new CustomEvent('slos-consents-accepted', {
		...
		shouldShowAgain() {
			try {
				const metadata = JSON.parse(
					localStorage.getItem('slos_consent_meta') || '{}'
				);
				...
				const isWithinGracePeriod =
					gracePeriodMs > 0 && timeSinceConsent < gracePeriodMs;

				// Phase 3.1.1: Check if policy version changed
				if (
					metadata.policyVersion &&
					metadata.policyVersion !== this.policyVersion
				) {
					// Grace period applies: if grace period is active, allow existing consent to continue
					if (isWithinGracePeriod) {
						console.log(
							'[SLOS] Policy version changed, but within grace period. Days remaining:',
							Math.ceil(
								(gracePeriodMs - timeSinceConsent) /
									(24 * 60 * 60 * 1000)
							)
						);
					...
					console.log(
						'[SLOS] Re-consent required: Policy version changed (grace period expired or disabled)'
					);
					return true;
				}
				...
				if (
					metadata.bannerVersion &&
					metadata.bannerVersion !== this.bannerVersion
				) {
					// Grace period applies: if grace period is active, allow existing consent to continue
					if (isWithinGracePeriod) {
						console.log(
							'[SLOS] Banner configuration changed, but within grace period. Days remaining:',
							Math.ceil(
								(gracePeriodMs - timeSinceConsent) /
									(24 * 60 * 60 * 1000)
							)
					...
					console.log(
						'[SLOS] Re-consent required: Banner configuration changed (grace period expired or disabled)'
					);
					return true;
				}
				...
				const newCategories = this.purposes.filter(
					(p) => !storedCategories.includes(p)
				);
				if (newCategories.length > 0) {
					console.log(
						'[SLOS] Re-consent required: New categories added (immediate, no grace period):',
						newCategories
					);
					return true;
				}
				...
				if (daysSince > expiryDays) {
					console.log(
						'[SLOS] Re-consent required: Consent expired after',
						expiryDays,
		...
		getPurposeDescription(purpose) {
			const descriptions = {
				...
				personalization: this.t(
					'descPersonalization',
					'Delivers personalized content based on your interests'
				),
			};
		...
		getCategoryDescription(category) {
			return (
				...
				this.getPurposeDescription(category)
			);
		}
		...
		getVendorList(category) {
			...
			const vendorItems = vendorData
				.map((vendor) => {
                    ...
                    <li class="slos-vendor-item">
                        ...
                        ${
							vendor.purpose
								? `<span class="slos-vendor-purpose">${vendor.purpose}</span>`
								: ''
						}
                    </li>
                `;
			...
			return `
                <div class="slos-vendor-list">
                    <strong class="slos-vendor-heading">${this.t(
						'services',
						'Services & Vendors'
					)}:</strong>
		...
		toggleCategoryDetails(purpose) {
			const detailsEl = document.getElementById(
				`slos-details-${purpose}`
			);
			const toggleBtn = document.querySelector(
				`.slos-expand-toggle[data-purpose="${purpose}"]`
			);

			if (!detailsEl || !toggleBtn) return;

			const isExpanded =
				detailsEl.getAttribute('aria-hidden') === 'false';
			...
			toggleBtn
				...
				.classList.toggle('dashicons-arrow-down-alt2', isExpanded);
			toggleBtn
				.querySelector('.dashicons')
				.classList.toggle('dashicons-arrow-up-alt2', !isExpanded);
		}
	}
