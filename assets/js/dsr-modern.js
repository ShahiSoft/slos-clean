	...
	const DSRModern = {
		...
		headerActions() {
			...
			$('#slos-new-request').on('click', function (e) {
				...
				const dsrFormUrl =
					typeof slosDSR !== 'undefined' && slosDSR.dsr_form_url
		...
		cardActions() {
			...
			$('.slos-request-card').on('click', function (e) {
				if (!$(e.target).closest('.slos-card-actions').length) {
					...
					console.log(
						'Card clicked:',
						$(this).find('.slos-request-id').text()
					);
				}
			});
		},
		...
		toasts() {
			...
			if (!$('#slos-toast-container').length) {
				$('body').append(
					'<div id="slos-toast-container" style="position: fixed; bottom: 20px; right: 20px; z-index: 9999;"></div>'
				);
			}
		},
...
function slOSShowActionsMenu(requestId) {
	...
	if (window.DSRModern) {
		window.DSRModern.showToast(
			'Action menu for request #' + requestId,
			'info'
		);
	}
}
