...
(function () {
	...
	const SLOSAutoFixProgress = {
		...
		show: function (options = {}) {
			...
			if (pageId > 0) {
				this.fetchPageScanResults(
					pageId,
					function (fixersWithIssues) {
		...
		cancel: function () {
			...
			this.state.activeRequests.forEach(
				function (xhr) {
			...
			this.state.fixers.forEach(
				function (fixer, index) {
		...
		getDefaultFixers: function () {
			...
			if (typeof window.slosautoFixConfig !== 'undefined' && Array.isArray( window.slosautoFixConfig.fixers )) {
				return window.slosautoFixConfig.fixers.map(
					function (fixer) {
						return {
							id: fixer.id,
							name: fixer.name,
		...
		updateFixer: function (fixerId, status, data = {}) {
			const fixer = this.state.fixers.find(
				function (f) {
					return f.id === fixerId;
				}
			);
		...
		displayElementErrors: function () {
			if ( ! this.state.elementErrors || this.state.elementErrors.length === 0) {
				return;
			}
