	...
	const SLOSAutoFixProgress = {
		...
		init() {
			...
			if (
				...
				console.log(
					'SLOSAutoFixProgress: Autofix is dormant, skipping initialization'
				);
				return;
			}
		...
		initializeFixerCatalog() {
			...
			if (
				typeof window.slosFixEngine !== 'undefined' &&
				Array.isArray(window.slosFixEngine.fixers)
			) {
				source = window.slosFixEngine.fixers;
			} else if (
				typeof window.slosautoFixConfig !== 'undefined' &&
				Array.isArray(window.slosautoFixConfig.fixers)
			) {
		 ...
		 */
		cacheElements() {
			this.elements.overlay = document.getElementById(
				'slos-autofix-progress-overlay'
			);
			this.elements.modal = this.elements.overlay.querySelector(
				'.slos-autofix-modal'
			);
			this.elements.closeBtn = this.elements.overlay.querySelector(
				'.slos-autofix-close'
			);
			this.elements.progressBar = this.elements.overlay.querySelector(
				'.slos-autofix-progress-bar'
			);
			this.elements.progressWrap = this.elements.overlay.querySelector(
				'.slos-autofix-progress-wrap'
			);
			this.elements.progressPercent = this.elements.overlay.querySelector(
				'.slos-autofix-progress-percent'
			);
			this.elements.progressStatus = this.elements.overlay.querySelector(
				'.slos-autofix-progress-status'
			);
			this.elements.fixerList = this.elements.overlay.querySelector(
				'.slos-autofix-fixer-list'
			);
			this.elements.summaryFixed = this.elements.overlay.querySelector(
				'[data-stat="fixed"]'
			);
			this.elements.summaryErrors = this.elements.overlay.querySelector(
				'[data-stat="errors"]'
			);
			this.elements.summarySkipped = this.elements.overlay.querySelector(
				'[data-stat="skipped"]'
			);
			this.elements.summaryPending = this.elements.overlay.querySelector(
				'[data-stat="pending"]'
			);
			this.elements.cancelBtn = this.elements.overlay.querySelector(
				'.slos-autofix-btn-cancel'
			);
			this.elements.closeActionBtn = this.elements.overlay.querySelector(
				'.slos-autofix-close-action'
			);
		},
		...
		createLiveRegion() {
			if (document.getElementById('slos-autofix-live-region')) {
				this.elements.liveRegion = document.getElementById(
					'slos-autofix-live-region'
				);
				return;
			}
		...
		bindEvents() {
			...
			if (this.elements.closeActionBtn) {
				this.elements.closeActionBtn.addEventListener(
					'click',
					function () {
						self.hide();
					}
				...
				this.elements.overlay.addEventListener('click', function (e) {
					if (e.target === self.elements.overlay) {
						if (self.state.isProcessing) {
							...
							const message =
								'Please wait for auto-fix to complete or use the Cancel button.';
		...
		handleClose() {
			if (this.state.isProcessing) {
				...
				const message =
					'Auto-fix is currently processing. Please use the Cancel button to stop, or wait for completion.';
		...
		showWithFixers(fixers, options) {
			...
			// Announce
			this.announce(
				'Auto-fix progress dialog opened. Processing ' +
					this.state.totalFixers +
					' fixers.'
			);
		 ...
		 * @param callback
		 */
		fetchPageScanResults(pageId, callback) {
			const self = this;
			...
			this.elements.progressStatus.textContent =
				'Analyzing page issues...';
			this.elements.fixerList.innerHTML =
				'<div style="padding: 20px; text-align: center; color: #94a3b8;">Checking for fixable issues...</div>';

			if (typeof jQuery === 'undefined') {
				// Fallback to default fixers if jQuery not available
				console.warn(
					'SLOSAutoFixProgress: jQuery not available, using all fixers'
				);
			...
			const timeoutId = setTimeout(function () {
				...
				console.warn(
					'SLOSAutoFixProgress: Timeout waiting for scan results, using all fixers'
				);
				isResolved = true;
			...
			const pendingRequest = jQuery.ajax({
				url:
					ajaxurl ||
					(typeof slosautoFixConfig !== 'undefined'
						? slosautoFixConfig.ajaxUrl
				...
				success(response) {
					...
					if (response.success && response.data) {
						...
						if (
							data.fixers &&
							data.fixers.length > 0 &&
							data.use_all_fixers !== true
						) {
							console.log(
								'SLOSAutoFixProgress: Using ' +
									data.fixers.length +
									' page-specific fixers'
							);
							callback(data.fixers);
						} else if (data.use_all_fixers === true) {
							// Explicit opt-in path to run all registered fixers.
							console.log(
								'SLOSAutoFixProgress: Backend signaled to use all fixers (' +
									(data.issue_count || 0) +
									' issues found)'
							);
							callback(self.getDefaultFixers());
							...
							// No fixable issues.
							console.log(
								'SLOSAutoFixProgress: No fixable issues found'
							);
							callback([]);
						}
					} else {
						...
						console.warn(
							'SLOSAutoFixProgress: No scan results or error, using all fixers'
						);
						callback(self.getDefaultFixers());
					}
				},
				error(xhr, status, error) {
					...
					console.error(
						'SLOSAutoFixProgress: AJAX error while loading page fixable issues',
						error
					);
					callback([]);
				},
		...
		cancel() {
			...
			this.announce(
				'Auto-fix cancelled. ' +
					this.state.fixedCount +
					' issues fixed before cancellation. All pending requests aborted.'
			);
		},
		...
		getDefaultFixers() {
			...
			const catalogIds = Object.keys(this.state.fixerCatalog || {});
			if (catalogIds.length > 0) {
				return catalogIds.map(
					function (id) {
						const base = this.state.fixerCatalog[id] || {};
						return {
							id: base.id || id,
							name: base.name || '',
				...
				groups[categoryKey].forEach(function (fixer) {
					...
					item.innerHTML = `
							...
							< div class    = "slos-autofix-fixer-info" >
								< div class = "slos-autofix-fixer-name" > ${self.escapeHtml(
									fixer.name
								)} < / div >
								< div class = "slos-autofix-fixer-desc" > ${self.escapeHtml(
									fixer.description
								)} < / div >
							< / div >
		...
		renderFixerList() {
			...
			this.state.fixers.forEach(function (fixer) {
				const item = self.elements.fixerList.querySelector(
					'[data-fixer-id="' + fixer.id + '"]'
				);
				if (!item) {
					return;
				}
				...
				const statusEl = item.querySelector(
					'.slos-autofix-fixer-status'
				);
				statusEl.className =
					'slos-autofix-fixer-status ' + fixer.status;
				...
				if (fixer.count > 0 || fixer.message) {
					...
					countEl.className =
						'slos-autofix-fixer-count ' + fixer.status;
					...
					if (fixer.status === 'success' && fixer.count > 0) {
						...
						countEl.setAttribute(
							'title',
							fixer.message || 'An error occurred during fixing'
						);
						...
						if (fixer.errorDetails) {
							...
							errorInfo.innerHTML =
								...
								self.escapeHtml(fixer.errorDetails);
							const infoDiv = item.querySelector(
								'.slos-autofix-fixer-info'
							);
							if (
								infoDiv &&
								!infoDiv.querySelector(
									'.slos-autofix-error-details'
								)
							) {
						...
						if (fixer.reason === 'no-issues') {
							label =
								fixer.message ||
								'No applicable issues on this page';
							...
							label =
								fixer.message || 'Not applicable on this page';
						}
		 ...
		 * @param data
		 */
		updateFixer(fixerId, status, data = {}) {
		 ...
		 */
		scrollToActiveFixer() {
			const activeItem = this.elements.fixerList.querySelector(
				'.slos-autofix-fixer-item.active'
			);
			if (activeItem) {
		...
		startProcessing(options) {
			...
			function processNext() {
				if (
					...
					currentIndex >= self.state.fixers.length
				) {
					self.complete();
					return;
				}
		 ...
		 * @param retryCount
		 */
		processFixer(fixer, pageId, content, callback, retryCount) {
			const self = this;
				...
				setTimeout(function () {
					...
					if (random > 0.15) {
						callback({
							success: true,
							data: {
								fixed_count: Math.floor(Math.random() * 10),
							},
						});
					} else if (random > 0.05) {
						callback({
			...
			const xhr = jQuery.ajax({
				...
				error(xhr, status, error) {
					...
					if (
						...
						console.log(
							'SLOSAutoFixProgress: Retrying fixer ' +
								fixer.id +
								' (attempt ' +
								(retryCount + 1) +
								')'
		...
		complete() {
			...
			if (this.state.isCancelled) {
				...
				this.elements.progressStatus.textContent =
					'Complete! No issues found to fix';
			} else if (
				this.state.errorCount > 0 &&
				this.state.fixedCount === 0
			) {
				...
				this.elements.progressStatus.textContent =
					'Complete! Fixed ' + this.state.fixedCount + ' issue(s)';
			}
			...
			if (this.state.fixedCount > 0) {
				announcement +=
					this.state.fixedCount +
					' ' +
					(this.state.fixedCount === 1 ? 'issue' : 'issues') +
					' fixed. ';
			} else {
			...
			if (this.state.errorCount > 0) {
				announcement +=
					this.state.errorCount +
					' ' +
					(this.state.errorCount === 1 ? 'error' : 'errors') +
					' occurred. ';
			}
			if (this.state.skippedCount > 0) {
				announcement +=
					this.state.skippedCount +
					' ' +
					(this.state.skippedCount === 1 ? 'fixer' : 'fixers') +
					' skipped.';
			}
		 ...
		 */
		displayRescanStats() {
			if (!this.state.rescanStats) {
				return;
			}

			// Find or create rescan stats container
			let statsContainer = this.elements.modal.querySelector(
				'.slos-autofix-rescan-stats'
			);
			if (!statsContainer) {
				statsContainer = document.createElement('div');
				statsContainer.className = 'slos-autofix-rescan-stats';
				statsContainer.style.cssText =
					'margin: 16px 0; padding: 12px; background: rgba(255,255,255,0.05); border-radius: 4px; font-size: 13px;';

				// Insert before fixer list
				const fixerListContainer = this.elements.fixerList.parentNode;
				if (fixerListContainer) {
					fixerListContainer.insertBefore(
						statsContainer,
						this.elements.fixerList
					);
				}
			}
			...
			statsContainer.innerHTML =
				'<div style="color: #94a3b8; margin-bottom: 8px;"><strong>Post-Fix Scan Results:</strong></div>' +
				'<div style="color: #f8fafc; display: flex; gap: 16px; flex-wrap: wrap;">' +
				'<span>Total Issues: <strong>' +
				(stats.totalIssues || 0) +
				'</strong></span>' +
				'<span>Remaining: <strong>' +
				(stats.remaining || 0) +
				'</strong></span>' +
				'<span>Resolved: <strong style="color: #10b981;">' +
				(stats.resolved || 0) +
				'</strong></span>' +
				'</div>';
		},
		 ...
		 */
		displayElementErrors() {
			if (
				!this.state.elementErrors ||
				this.state.elementErrors.length === 0
			) {
				return;
			}

			// Find or create error details container
			let errorContainer = this.elements.modal.querySelector(
				'.slos-autofix-all-errors'
			);
			if (!errorContainer) {
				errorContainer = document.createElement('div');
				errorContainer.className = 'slos-autofix-all-errors';
				errorContainer.style.cssText =
					'margin: 16px 0; padding: 12px; background: rgba(239, 68, 68, 0.1); border-left: 3px solid #ef4444; border-radius: 4px; font-size: 13px;';

				// Insert after rescan stats or before fixer list
				const statsContainer = this.elements.modal.querySelector(
					'.slos-autofix-rescan-stats'
				);
				if (statsContainer) {
					...
					const fixerListContainer =
						this.elements.fixerList.parentNode;
					if (fixerListContainer) {
						fixerListContainer.insertBefore(
							errorContainer,
							this.elements.fixerList
			...
			let errorsHTML =
				'<div style="color: #fca5a5; margin-bottom: 8px;"><strong>⚠ Errors Encountered (' +
				this.state.elementErrors.length +
				'):</strong></div>';
			errorsHTML +=
				'<ul style="margin: 0; padding-left: 20px; color: #fecaca;">';

			this.state.elementErrors.forEach(
				function (error) {
					errorsHTML +=
						'<li style="margin: 4px 0;"><strong>' +
						this.escapeHtml(error.fixer) +
						':</strong> ' +
		...
		loadProgress(pageId) {
			...
			if (saved) {
				try {
					...
					console.error(
						'SLOSAutoFixProgress: Failed to load progress:',
						e
					);
					localStorage.removeItem(storageKey);
				}
		...
		saveProgress(pageId) {
			...
			try {
				...
				console.warn(
					'SLOSAutoFixProgress: Failed to save progress:',
					e
				);
			}
		},
		...
		clearProgress(pageId) {
			...
			try {
				...
				console.warn(
					'SLOSAutoFixProgress: Failed to clear progress:',
					e
				);
			}
		},
