...
use ShahiLegalFlowSuite\Services\Document_Generator;
use ShahiLegalFlowSuite\Services\Export_Manager;
...
class Document_Hub_Controller {
	...
	public function render() {
		...
		if ( file_exists( $template_path ) ) {
			...
			echo '<div class="wrap"><p>' . esc_html__( 'Template not found.', 'shahi-legalflowsuite' ) . '</p></div>';
		}
	}
	...
	public function enqueue_assets() {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'    => array(
					'generating'            => __( 'Generating document...', 'shahi-legalflowsuite' ),
					'generated'             => __( 'Document generated successfully!', 'shahi-legalflowsuite' ),
					'generateError'         => __( 'Error generating document. Please try again.', 'shahi-legalflowsuite' ),
					'publishing'            => __( 'Publishing document...', 'shahi-legalflowsuite' ),
					'published'             => __( 'Document published successfully!', 'shahi-legalflowsuite' ),
					'publishError'          => __( 'Error publishing document. Please try again.', 'shahi-legalflowsuite' ),
					'regenerating'          => __( 'Regenerating document...', 'shahi-legalflowsuite' ),
					'regenerated'           => __( 'Document regenerated successfully!', 'shahi-legalflowsuite' ),
					'regenerateError'       => __( 'Error regenerating document. Please try again.', 'shahi-legalflowsuite' ),
					'confirmRegenerate'     => __( 'This will regenerate the document from your current profile. Any manual edits will be saved in version history. Continue?', 'shahi-legalflowsuite' ),
					'confirmDelete'         => __( 'Are you sure you want to delete this document? This action cannot be undone.', 'shahi-legalflowsuite' ),
					'confirmBulkRegenerate' => __( 'Regenerate all outdated documents? This will update them with your current profile data.', 'shahi-legalflowsuite' ),
					'profileIncomplete'     => __( 'Your company profile is incomplete. Complete it first for better document generation.', 'shahi-legalflowsuite' ),
					'profileComplete'       => __( 'Your profile is complete and ready for document generation.', 'shahi-legalflowsuite' ),
					'missingFields'         => __( 'This document requires the following profile fields:', 'shahi-legalflowsuite' ),
					'goToProfile'           => __( 'Go to Profile', 'shahi-legalflowsuite' ),
					'generateAnyway'        => __( 'Generate Anyway', 'shahi-legalflowsuite' ),
					'completeProfile'       => __( 'Complete Profile', 'shahi-legalflowsuite' ),
					'regenerateWarning'     => __( 'This will create a new version. The previous version will be saved in history.', 'shahi-legalflowsuite' ),
					'loading'               => __( 'Loading...', 'shahi-legalflowsuite' ),
					'close'                 => __( 'Close', 'shahi-legalflowsuite' ),
					'error'                 => __( 'An error occurred. Please try again.', 'shahi-legalflowsuite' ),
					'documentPreview'       => __( 'Document Preview', 'shahi-legalflowsuite' ),
					'noVersions'            => __( 'No version history available.', 'shahi-legalflowsuite' ),
					'noReason'              => __( 'No description provided', 'shahi-legalflowsuite' ),
					'shortcodeCopied'       => __( 'Shortcode copied to clipboard!', 'shahi-legalflowsuite' ),
					'downloadStarted'       => __( 'Download started...', 'shahi-legalflowsuite' ),
					'downloadError'         => __( 'Failed to download document. Please try again.', 'shahi-legalflowsuite' ),
					'noDocsToExport'        => __( 'No documents available to export.', 'shahi-legalflowsuite' ),
					'exportSuccess'         => __( 'Documents exported successfully!', 'shahi-legalflowsuite' ),
					'exportError'           => __( 'Failed to export documents. Please try again.', 'shahi-legalflowsuite' ),
				),
			)
		);
	}
	...
	public function ajax_publish_document() {
		...
		if ( ! $updated ) {
			wp_send_json_error( array( 'message' => __( 'Failed to publish document.', 'shahi-legalflowsuite' ) ) );
		}

		wp_send_json_success(
			array(
				'message' => __( 'Document published successfully!', 'shahi-legalflowsuite' ),
				'doc_id'  => $document->id,
				'status'  => 'published',
				'refresh' => true,
			)
	...
	public function ajax_delete_document() {
		...
		if ( ! $deleted ) {
			wp_send_json_error( array( 'message' => __( 'Failed to delete document.', 'shahi-legalflowsuite' ) ) );
		}

		wp_send_json_success(
			array(
				'message' => __( 'Document deleted successfully!', 'shahi-legalflowsuite' ),
				'refresh' => true,
			)
		);
	}
		...
		switch ( $action ) {
			case 'regenerate_outdated':
				...
				if ( empty( $outdated ) ) {
					wp_send_json_success(
						array(
							'message' => __( 'No outdated documents to regenerate.', 'shahi-legalflowsuite' ),
							'refresh' => false,
						)
					);
				}
				...
				$message = sprintf(
					...
					__( 'Regenerated %1$d of %2$d documents.', 'shahi-legalflowsuite' ),
					$result['success'],
					$result['total']
				);
				...
				if ( $result['failed'] > 0 ) {
					$message .= ' ' . sprintf(
						...
						__( '%d failed.', 'shahi-legalflowsuite' ),
						$result['failed']
					);
				}
			...
			case 'export_all':
				...
				if ( empty( $documents ) ) {
					wp_send_json_error( array( 'message' => __( 'No documents to export.', 'shahi-legalflowsuite' ) ) );
				}
				...
				if ( empty( $doc_ids ) ) {
					wp_send_json_error( array( 'message' => __( 'No valid documents to export.', 'shahi-legalflowsuite' ) ) );
				}
				...
				if ( ! $result['success'] ) {
					wp_send_json_error( array( 'message' => $result['message'] ?? __( 'Export failed.', 'shahi-legalflowsuite' ) ) );
				}
				...
				wp_send_json_success(
					array(
						'message'      => sprintf(
							...
							__( 'Successfully exported %d documents.', 'shahi-legalflowsuite' ),
							count( $doc_ids )
						),
			...
			default:
				wp_send_json_error( array( 'message' => __( 'Invalid action.', 'shahi-legalflowsuite' ) ) );
		}
	}
	...
	public function ajax_get_document_preview() {
		...
		if ( 'generated' === $preview_mode ) {
			...
			if ( ! $preview['success'] ) {
				wp_send_json_error( array( 'message' => $preview['message'] ?? __( 'Preview unavailable.', 'shahi-legalflowsuite' ) ) );
			}
			...
			wp_send_json_success(
				array(
					...
					'version'  => __( 'Preview', 'shahi-legalflowsuite' ),
			...
			if ( $preview['success'] ) {
				...
				wp_send_json_success(
					array(
						'content'    => wp_kses_post( $preview['content'] ),
						'title'      => esc_html( $type_config['title'] ?? $doc_type ),
						'version'    => __( 'Preview (not generated)', 'shahi-legalflowsuite' ),
						'is_preview' => true,
					)
				);
			}
	...
	public function ajax_export_document() {
		...
		if ( ! $result['success'] ) {
			wp_send_json_error(
				array(
					'message' => $result['message'] ?? __( 'Export failed.', 'shahi-legalflowsuite' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message'      => __( 'Document exported successfully.', 'shahi-legalflowsuite' ),
				'download_url' => $result['download_url'],
				'filename'     => $result['filename'],
				'format'       => $format,
			)
	...
	public function ajax_export_bulk() {
		...
		if ( ! $result['success'] ) {
			wp_send_json_error(
				array(
					'message' => $result['message'] ?? __( 'Bulk export failed.', 'shahi-legalflowsuite' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message'        => sprintf(
					/* translators: %d: document count */
					__( 'Successfully exported %d documents as ZIP.', 'shahi-legalflowsuite' ),
					count( $doc_ids )
				),
				'download_url'   => $result['download_url'],
				'filename'       => $result['filename'],
		...
		try {
			...
			wp_send_json_success(
				array(
					...
					'message'  => __( 'Document generated as draft. Review and publish when ready.', 'shahi-legalflowsuite' ),
				)
			);
		} catch ( \Exception $e ) {
	...
	public function ajax_get_version_history() {
		...
		if ( $doc_id <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid document ID', 'shahi-legalflowsuite' ) ), 400 );
		}
	...
	public function ajax_restore_version() {
		...
		if ( $version_id <= 0 || $doc_id <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid parameters', 'shahi-legalflowsuite' ) ), 400 );
		}
		...
		wp_send_json_success(
			array(
				...
				'message'  => __( 'Version restored as new draft', 'shahi-legalflowsuite' ),
			)
		);
	}
	...
	public function ajax_compare_versions() {
		...
		if ( $version_id <= 0 || $compare_id <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid version IDs', 'shahi-legalflowsuite' ) ), 400 );
		}
		...
		if ( class_exists( 'WP_Text_Diff_Renderer_Table' ) ) {
			$text_diff = wp_text_diff(
				...
				array(
					'title'       => '',
					'title_left'  => __( 'Previous', 'shahi-legalflowsuite' ),
					'title_right' => __( 'Current', 'shahi-legalflowsuite' ),
				)
			);
			...
			$diff_html .= '<div class="slos-gen-diff-right"><h4>' . esc_html__( 'Current Version', 'shahi-legalflowsuite' ) . '</h4>';
	...
	public function ajax_save_document_edit() {
		...
		$this->doc_repository->create_version(
			...
			array(
				'content'       => $content,
				'change_reason' => __( 'Manual edit', 'shahi-legalflowsuite' ),
			)
		);

		wp_send_json_success(
			array(
				'message' => __( 'Document saved successfully.', 'shahi-legalflowsuite' ),
			)
		);
	}
}
