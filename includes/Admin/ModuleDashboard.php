...
class ModuleDashboard {
	...
	public function render() {
		...
		wp_enqueue_style(
			'shahi-legalflowsuite-modules',
			SHAHI_LEGALFLOWSUITE_PLUGIN_URL . 'assets/css/admin-modules-new.css',
			array( 'shahi-legalflowsuite-dashboard' ),
			SHAHI_LEGALFLOWSUITE_VERSION
		);
		...
		// Load template..
		include SHAHI_LEGALFLOWSUITE_PATH . 'templates/admin/module-dashboard.php';
	}
	...
	public function ajax_toggle_module() {
		...
		if ( ! Security::verify_nonce( $_POST['nonce'] ?? '', 'shahi_module_dashboard' ) ) {
			wp_send_json_error( array( 'message' => __( 'Security check failed.', 'shahi-legalflowsuite' ) ) );
		}

		if ( ! current_user_can( 'manage_shahi_modules' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'shahi-legalflowsuite' ) ) );
		}

		$module_slug = isset( $_POST['module'] ) ? sanitize_text_field( wp_unslash( <?php
/**
 * Module Dashboard Admin Page
 *
 * Premium module management dashboard with advanced visualizations and controls.
...
class ModuleDashboard {
	...
	public function ajax_toggle_module() {
		...
		if ( empty( $module_slug ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid module.', 'shahi-legalflowsuite' ) ) );
		}
		...
		if ( $result ) {
			...
			wp_send_json_success(
				array(
					'message' => $enabled ? __( 'Module activated successfully.', 'shahi-legalflowsuite' ) : __( 'Module deactivated successfully.', 'shahi-legalflowsuite' ),
					'enabled' => $enabled,
				)
			);
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to toggle module.', 'shahi-legalflowsuite' ) ) );
		}
	}
	...
	public function ajax_bulk_action() {
		...
		if ( empty( $action ) || empty( $modules ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid bulk action.', 'shahi-legalflowsuite' ) ) );
		}
...
POST['module'] ) ) : '';
	...
	public function ajax_bulk_action() {
		...
		$action  = isset( $_POST['action_type'] ) ? sanitize_text_field( wp_unslash( <?php
/**
 * Module Dashboard Admin Page
 *
 * Premium module management dashboard with advanced visualizations and controls.
