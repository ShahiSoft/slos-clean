...
class Activator {
	...
	private static function set_default_options() {
		...
		if ( ! get_option( 'shahi_legalflowsuite_uninstall' ) ) {
			$uninstall_prefs = array(
				'preserve_data'        => true,
				'preserve_settings'    => true,
				'preserve_analytics'   => true,
				'preserve_documents'   => true,
				'preserve_consent_log' => true,
			);
			add_option( 'shahi_legalflowsuite_uninstall', $uninstall_prefs );
		}
	...
	private static function add_analytics_indexes() {
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $events_table ) ) === $events_table ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $analytics_table ) ) === $analytics_table ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	private static function create_index_if_not_exists( $table_name, $index_name, $columns ) {
		...
		// Create the index..
		$sql = "ALTER TABLE `{$escaped_table}` ADD INDEX `{$escaped_index}` ({$column_list})";
