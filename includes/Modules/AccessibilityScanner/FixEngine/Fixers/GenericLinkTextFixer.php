...
final class GenericLinkTextFixer extends AbstractFixer {
	...
	private function find_nearby_heading( \DOMElement $link ): string {
		...
		while ( $node && $depth < 5 ) {
			...
			if ( $node instanceof \DOMElement && in_array( $node->nodeName, [ 'article', 'section', 'div' ] ) ) {
	...
	private function combine_with_original( string $original, string $context ): string {
		...
		$action_words = [ 'read', 'learn', 'view', 'see', 'click', 'go to', 'visit' ];
		...
		if ( in_array( strtolower( $original ), [ 'read more', 'learn more', 'see more', 'view more' ] ) ) {
			$action = explode( ' ', $original )[0];
			return sprintf( '%s about %s', ucfirst( $action ), $context );
		}

		if ( in_array( strtolower( $original ), [ 'continue', 'continue reading' ] ) ) {
