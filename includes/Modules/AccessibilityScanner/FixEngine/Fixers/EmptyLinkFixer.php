...
final class EmptyLinkFixer extends AbstractFixer {
	...
	private function detect_action_link( string $href, \DOMElement $link ): ?string {
		if ( strpos( $href, 'mailto:' ) === 0 ) {
			...
			return sprintf( __( 'Send email to %s', 'shahi-legalflowsuite' ), $email );
		}

		if ( strpos( $href, 'tel:' ) === 0 ) {
			$phone = str_replace( 'tel:', '', $href );
			return sprintf( __( 'Call %s', 'shahi-legalflowsuite' ), $phone );
		}

		if ( strpos( $href, '#' ) === 0 ) {
			$anchor = substr( $href, 1 );
			if ( ! empty( $anchor ) ) {
				$readable = str_replace( [ '-', '_' ], ' ', $anchor );
				return sprintf( __( 'Jump to %s', 'shahi-legalflowsuite' ), ucwords( $readable ) );
			}
		}
	...
	private function generate_from_url( string $href ): string {
		...
		if ( ! empty( $parsed['host'] ) ) {
			...
			return sprintf( __( 'Visit %s', 'shahi-legalflowsuite' ), $domain );
		}

		if ( ! empty( $parsed['path'] ) ) {
			$path = basename( $parsed['path'] );
			$readable = str_replace( [ '-', '_' ], ' ', preg_replace( '/\.[^.]+$/', '', $path ) );
			if ( ! empty( $readable ) ) {
