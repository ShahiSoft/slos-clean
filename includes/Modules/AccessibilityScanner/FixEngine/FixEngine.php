...
final class FixEngine {
	 ...
	 * @return FixResult
	 */
	public function fix_with_fixer( string $content, string $fixer_id, array $options = [] ): FixResult {
		$this->initialize();
	 ...
	 * @return FixSession
	 */
	public function fix_content( string $content, array $fixer_ids = [], array $options = [] ): FixSession {
		$this->initialize();
	 ...
	 * @return FixSession
	 */
	public function fix_post( int $post_id, array $fixer_ids = [], array $options = [] ): FixSession {
		$this->initialize();
	 ...
	 */
	public function preview_fixes( int $post_id, array $fixer_ids = [] ): array {
		$post = get_post( $post_id );
		...
		$session = $this->fix_content( $post->post_content, $fixer_ids, [
			'post_id'      => $post_id,
			'save_content' => false,
		] );
