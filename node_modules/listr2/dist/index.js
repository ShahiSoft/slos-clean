...
var ListrEnvironmentVariables = /* @__PURE__ */ ((
	ListrEnvironmentVariables2
) => {
...
function isUnicodeSupported() {
	return (
		!!process.env['LISTR_FORCE_UNICODE' /* FORCE_UNICODE */] ||
...
var color = createColors({
	useColor: !process.env['LISTR_DISABLE_COLOR' /* DISABLE_COLOR */],
});
	...
	suffix(message, ...suffixes) {
		suffixes.filter(Boolean).forEach((suffix) => {
			...
			if (typeof suffix === 'string') {
				...
				message += this.wrap(
					typeof suffix.field === 'function'
						? suffix.field(...suffix.args)
						: suffix.field,
					{
	...
	prefix(message, ...prefixes) {
		prefixes.filter(Boolean).forEach((prefix) => {
			...
			if (typeof prefix === 'string') {
				...
				message =
					this.wrap(
						typeof prefix.field === 'function'
							? prefix.field(...prefix.args)
							: prefix.field,
						{
				...
				return this.style(
					...
					this.fields(m, {
						prefix: Array.isArray(options?.prefix)
							? options.prefix
							: [options?.prefix],
						suffix: Array.isArray(options?.suffix)
							? options.suffix
							: [options?.suffix],
					})
				);
...
var _ProcessOutput = class _ProcessOutput {
	...
	release() {
		const output = Object.entries(this.stream)
			...
			.filter((output2) => this.options.dump.includes(output2.name))
			.flatMap((output2) => output2.buffer)
...
async function createPrompt(options, settings) {
	...
	options = options.map((option) => {
		return {
			...
			stdout:
				this instanceof TaskWrapper
					? (settings?.stdout ?? this.stdout('PROMPT' /* PROMPT */))
					: process.stdout,
		};
	});
	let enquirer;
	if (settings?.enquirer) {
		...
		try {
			enquirer = await import('enquirer').then((imported) =>
				imported.default ? new imported.default() : new imported()
			);
		} catch (e) {
			if (this instanceof TaskWrapper) {
				this.task.prompt = new PromptError(
	...
	if (this instanceof TaskWrapper) {
		...
		enquirer
			.on('prompt', (prompt) => (this.task.prompt = prompt))
			.on('submit', () => (this.task.prompt = void 0));
		this.task.on('STATE' /* STATE */, (event) => {
			if (
				event === 'SKIPPED' /* SKIPPED */ &&
				this.task.prompt &&
				!(this.task.prompt instanceof PromptError)
			) {
...
var ListrDefaultRendererLogLevels = /* @__PURE__ */ ((
	ListrDefaultRendererLogLevels2
) => {
	ListrDefaultRendererLogLevels2['SKIPPED_WITH_COLLAPSE'] =
		'SKIPPED_WITH_COLLAPSE';
	ListrDefaultRendererLogLevels2['SKIPPED_WITHOUT_COLLAPSE'] =
		'SKIPPED_WITHOUT_COLLAPSE';
	ListrDefaultRendererLogLevels2['OUTPUT'] = 'OUTPUT';
	ListrDefaultRendererLogLevels2['OUTPUT_WITH_BOTTOMBAR'] =
		'OUTPUT_WITH_BOTTOMBAR';
	ListrDefaultRendererLogLevels2['PENDING'] = 'PENDING';
	ListrDefaultRendererLogLevels2['COMPLETED'] = 'COMPLETED';
	ListrDefaultRendererLogLevels2['COMPLETED_WITH_FAILED_SUBTASKS'] =
		'COMPLETED_WITH_FAILED_SUBTASKS';
	ListrDefaultRendererLogLevels2['COMPLETED_WITH_FAILED_SISTER_TASKS'] =
	...
	ListrDefaultRendererLogLevels2['FAILED_WITH_FAILED_SUBTASKS'] =
...
var LISTR_DEFAULT_RENDERER_STYLE = {
	icon: {
		...
		['COMPLETED_WITH_FAILED_SUBTASKS' /* COMPLETED_WITH_FAILED_SUBTASKS */]:
			figures.warning,
		['COMPLETED_WITH_SISTER_TASKS_FAILED' /* COMPLETED_WITH_FAILED_SISTER_TASKS */]:
...
var _DefaultRenderer = class _DefaultRenderer {
	...
	style(task, output = false) {
		...
		if (task.hasSubtasks()) {
			if (
				...
				return this.logger.icon(
					'FAILED_WITH_SUBTASKS' /* FAILED_WITH_FAILED_SUBTASKS */
				);
			}
		}
	...
	format(message, icon, level) {
		...
		let parsed;
		const columns =
			(process.stdout.columns ?? 80) - level * this.options.indentation - 2;
		switch (this.options.formatOutput) {
			...
			default:
				throw new ListrRendererError(
					'Format option for the renderer is wrong.'
				);
		}
	...
	renderer(tasks, level = 0) {
		return tasks.flatMap((task) => {
			...
			if (task.isPrompt()) {
				if (this.activePrompt && this.activePrompt !== task.id) {
					...
					task.on('STATE' /* STATE */, (state) => {
						if (
							state === 'PROMPT_COMPLETED' /* PROMPT_COMPLETED */ ||
							task.hasFinalized() ||
							task.hasReset()
						) {
							this.prompt = null;
					...
					if (!this.bottom.has(task.id)) {
						this.bottom.set(
							...
							new ProcessOutputBuffer({
								limit:
									typeof rendererTaskOptions.bottomBar === 'boolean'
										? 1
			...
			if (
				...
				task.hasSubtasks() &&
				(task.isPending() ||
					(task.hasFinalized() && !task.hasTitle()) || // have to be completed and have subtasks
					(task.isCompleted() &&
						rendererOptions.collapseSubtasks === false &&
						!task.subtasks.some(
							(subtask) => subtask.rendererOptions.collapseSubtasks === true
						)) || // if any of the subtasks have the collapse option of
					task.subtasks.some(
						(subtask) => subtask.rendererOptions.collapseSubtasks === false
	...
	calculate(task) {
		if (
			this.cache.rendererOptions.has(task.id) &&
			this.cache.rendererTaskOptions.has(task.id)
		) {
			return;
		}
...
var _SimpleRenderer = class _SimpleRenderer {
	constructor(tasks, options) {
		...
		this.logger =
			this.options.logger ??
			new ListrLogger({ useIcons: true, toStderr: LISTR_LOGGER_STDERR_LEVELS });
		this.logger.options.icon = this.options.icon;
		this.logger.options.color = this.options.color;
	...
	renderer(tasks) {
		tasks.forEach((task) => {
			...
			if (this.options.state) {
				task.on('STATE' /* STATE */, (state) => {
					this.logger.toStdout(
						this.serializer.serialize('STATE' /* STATE */, state, task)
					);
				});
			}
			if (this.options.output) {
				task.on('OUTPUT' /* OUTPUT */, (data) => {
					this.logger.toStdout(
						this.serializer.serialize('OUTPUT' /* OUTPUT */, data, task)
					);
				});
			}
			if (this.options.prompt) {
				task.on('PROMPT' /* PROMPT */, (prompt) => {
					this.logger.toStdout(
						this.serializer.serialize('PROMPT' /* PROMPT */, prompt, task)
					);
				});
			}
			if (this.options.title) {
				task.on('TITLE' /* TITLE */, (title) => {
					this.logger.toStdout(
						this.serializer.serialize('TITLE' /* TITLE */, title, task)
					);
				});
			}
			task.on('MESSAGE' /* MESSAGE */, (message) => {
				...
				if (Object.keys(parsed).length > 0) {
					...
					if (
						this.options.messagesToStderr.some((state) =>
							Object.keys(parsed).includes(state)
						)
					) {
...
function getRenderer(options) {
	...
	if (
		!isRendererSupported(r.renderer) ||
		assertFunctionOrSelf(options?.fallbackRendererCondition)
	) {
		return {
...
var _TaskWrapper = class _TaskWrapper {
	...
	skip(message, ...metadata) {
		...
		if (message) {
			this.task.message$ = {
				skip: message ? splat(message, ...metadata) : this.task?.title,
			};
		}
	}
	/**
...
var _Task = class _Task extends ListrTaskEventManager {
	...
	set promptOutput$(data) {
		...
		if (cleanseAnsi(data)) {
			this.listr.events.emit(
				'SHOUD_REFRESH_RENDER' /* SHOULD_REFRESH_RENDER */
			);
		}
	}
	/**
	...
	async check(ctx) {
		if (this.state === 'WAITING' /* WAITING */) {
			this.enabled = await assertFunctionOrSelf(
	...
	isPrompt() {
		return (
			...
			this.state === 'PROMPT_COMPLETED' /* PROMPT_COMPLETED */
		);
	}
	...
	async run(context, wrapper) {
		...
		const skipped = await assertFunctionOrSelf(
			this.task?.skip ?? false,
			context
		);
		if (skipped) {
		...
		try {
			const retryCount =
				typeof this.task?.retry === 'number' && this.task.retry > 0
					...
					: typeof this.task?.retry === 'object' && this.task.retry.tries > 0
						...
						: 1;
			const retryDelay =
				typeof this.task.retry === 'object' && this.task.retry.delay;
			...
			if (this.task?.rollback) {
				...
				if (
					...
					(await assertFunctionOrSelf(this.task?.exitOnError, context)) !==
						false
				) {
...
var _Listr = class _Listr {
	...
	async run(context) {
		...
		try {
			await Promise.all(
				this.tasks.map((task) => this.concurrency.add(() => this.runTask(task)))
			);
			this.renderer.end();
		} catch (err) {
	...
	generate(tasks) {
		...
		return tasks.map(
			(task) =>
				new Task(this, task, this.options, { ...this.rendererClassOptions })
		);
	}
	async runTask(task) {
