...
> {
	...
	on<E extends Event = Event>(
		dispatch: E,
		handler: (data: EventData<E, Map>) => void
	): void;
	once<E extends Event = Event>(
		dispatch: E,
		handler: (data: EventData<E, Map>) => void
	): void;
 ...
 */
interface ListrLoggerOptions<Levels extends string>
	extends ProcessOutputRendererOptions, ListrLoggerStyleMap<Levels | string> {
	/**
...
interface LoggerFieldOptions<MultipleOnly extends boolean = false> {
	 ...
	 */
	prefix?: MultipleOnly extends false
		? LoggerField | LoggerField[]
		: LoggerField[];
	/**
	 * Suffix fields for the log entry.
	 */
	suffix?: MultipleOnly extends false
		? LoggerField | LoggerField[]
		: LoggerField[];
}
...
declare class ProcessOutputBuffer {
	...
	write(
		...
		...args:
			| [(string | undefined)?, ((err?: Error) => void)?]
			| [((err?: Error) => void)?]
...
declare function createWritable(
...
type PromptOptions<T extends boolean = false> =
	| Unionize<{
...
type PromptTypes =
	...
	| 'Text'
	| 'Toggle';
type PromptOptionsType<T> = T extends keyof PromptOptionsMap
	? PromptOptionsMap[T]
	: T extends string
		? BasePromptOptions & Record<PropertyKey, unknown>
		: any;
declare class PromptOptionsMap implements Record<
	PromptTypes,
...
interface PromptInstance extends Omit<
	BasePromptOptions,
	'onCancel' | 'onSubmit'
> {
	submit: () => void;
...
declare function getRendererClass(
 ...
 */
declare function assertFunctionOrSelf<T>(
	functionOrSelf: T,
...
declare class TaskWrapper<Ctx, Renderer extends ListrRendererFactory> {
	...
	newListr<NewCtx = Ctx>(
		task:
			...
			| ListrTask<NewCtx, Renderer>[]
			| ((
					parent: Omit<this, 'skip' | 'enabled'>
			  ) => ListrTask<NewCtx, Renderer> | ListrTask<NewCtx, Renderer>[]),
	 ...
	 */
	stdout(
		type?: ListrTaskEventType.OUTPUT | ListrTaskEventType.PROMPT
	): NodeJS.WritableStream;
	/** Run this task. */
	run(ctx: Ctx): Promise<void>;
}

declare class ListrTaskEventManager extends EventManager<
	ListrTaskEventType,
	ListrTaskEventMap
> {}
...
declare class ListrEventManager extends EventManager<
	ListrEventType,
	ListrEventMap
> {}
...
type ListrDefaultRendererOptionsStyle =
...
type TestRendererSerializerTaskKeys = Extract<
...
declare class TestRendererSerializer {
	...
	serialize<T extends ListrTaskEventType>(
		event: T,
		data: ListrTaskEventMap[T],
		task?: ListrTestRendererTask
	): string;
	generate<T extends ListrTaskEventType>(
		event: T,
		data: ListrTaskEventMap[T],
		task?: ListrTestRendererTask
...
declare class VerboseRenderer implements ListrRenderer {
	...
	private readonly cache;
	constructor(
		tasks: ListrVerboseRendererTask[],
 ...
 */
type ListrGetRendererValueFromClass<T extends ListrRendererFactory> =
	T extends DefaultRenderer
		? ListrDefaultRendererValue
 ...
 */
type ListrGetRendererOptions<T extends ListrRendererValue> =
	T extends ListrRendererValue
		? ListrGetRendererClassFromValue<T>['rendererOptions']
		: never;
/**
 * Returns renderer per-task options depending on the renderer type.
 */
type ListrGetRendererTaskOptions<T extends ListrRendererValue> =
	T extends ListrRendererValue
		? ListrGetRendererClassFromValue<T>['rendererTaskOptions']
		: never;
 ...
 */
interface ListrBaseClassOptions<
	Ctx = ListrContext,
 ...
 */
interface ListrSubClassOptions<
	Ctx = ListrContext,
	Renderer extends ListrRendererValue = ListrPrimaryRendererValue,
>
	extends
 ...
 */
declare class ListrEventMap
	extends BaseEventMap
	implements EventMap<ListrEventType>
{
...
> {
	...
	skip?:
		| boolean
		| string
		| ((ctx: Ctx) => boolean | string | Promise<boolean | string>);
	/**
 ...
 */
type ListrTaskFn<Ctx, Renderer extends ListrRendererFactory> = (
	ctx: Ctx,
	task: TaskWrapper<Ctx, Renderer>
 ...
 */
declare class ListrTaskEventMap
	extends BaseEventMap
	implements EventMap<ListrTaskEventType>
{
...
> {
	...
	>(
		tasks: ListrTask<
			InjectCtx,
			ListrGetRendererClassFromValue<InjectRenderer>
		>[],
