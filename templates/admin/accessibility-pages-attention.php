            ...
            <table class="slos-pages-table" style="width: 100%; border-collapse: collapse; background: var(--slos-bg-card); border-radius: 6px;">
                ...
                <tbody>
                    <?php foreach ( $pages_with_issues as $page ): 
                        ...
                        $priority_label = array( 'high' => 'High', 'medium' => 'Medium', 'low' => 'Low' )[ $priority ] ?? 'Low';
                    ...
                    <tr class="slos-page-row" data-page-id="<?php echo esc_attr( $post_id ); ?>" data-priority="<?php echo esc_attr( $priority ); ?>" style="border-bottom: 1px solid var(--slos-border); transition: background-color 0.2s ease;">
                        ...
                        <td class="issues-count" style="padding: 16px; text-align: center;">
                            <div style="display: inline-flex; align-items: center; justify-content: center; min-width: 40px; height: 32px; padding: 0 12px; background: <?php echo $issues > 15 ? 'rgba(239, 68, 68, 0.15)' : 'rgba(251, 191, 36, 0.15)'; ?>; border-radius: 6px; font-weight: 600; font-size: 14px; color: <?php echo $issues > 15 ? 'var(--slos-error)' : 'var(--slos-warning)'; ?>;">
                        ...
                        <td class="score-cell" style="padding: 16px; text-align: center;">
                            <div style="display: inline-flex; align-items: center; gap: 8px; padding: 6px 12px; background: <?php echo $score >= 80 ? 'rgba(34, 197, 94, 0.15)' : ( $score >= 60 ? 'rgba(251, 191, 36, 0.15)' : 'rgba(239, 68, 68, 0.15)' ); ?>; border-radius: 6px;">
                                <div style="width: 40px; height: 6px; background: var(--slos-bg-input); border-radius: 3px; position: relative; overflow: hidden;">
                                    <div style="position: absolute; left: 0; top: 0; height: 100%; width: <?php echo $score; ?>%; background: <?php echo $score >= 80 ? 'var(--slos-success)' : ( $score >= 60 ? 'var(--slos-warning)' : 'var(--slos-error)' ); ?>; border-radius: 3px; transition: width 0.3s ease;"></div>
                                </div>
                                <span style="font-weight: 700; font-size: 13px; color: <?php echo $score >= 80 ? 'var(--slos-success)' : ( $score >= 60 ? 'var(--slos-warning)' : 'var(--slos-error)' ); ?>; min-width: 38px;">
                                    <?php echo esc_html( $score ); ?>%
                                </span>
                            </div>
                        </td>
                        <td class="priority-cell" style="padding: 16px; text-align: center;">
                            <span style="display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 12px; text-transform: uppercase; <?php 
                                if ( $priority === 'high' ) {
                                    echo 'background: rgba(239, 68, 68, 0.15); color: var(--slos-error); border: 1px solid rgba(239, 68, 68, 0.3);';
                                } elseif ( $priority === 'medium' ) {
                                    echo 'background: rgba(251, 191, 36, 0.15); color: var(--slos-warning); border: 1px solid rgba(251, 191, 36, 0.3);';
                                } else {
                                    echo 'background: rgba(59, 130, 246, 0.15); color: var(--slos-accent); border: 1px solid rgba(59, 130, 246, 0.3);';
                                }
                            ?>">
                                <span class="dashicons dashicons-<?php echo $priority === 'high' ? 'arrow-up-alt' : ( $priority === 'medium' ? 'minus' : 'arrow-down-alt' ); ?>" style="font-size: 14px;"></span>
