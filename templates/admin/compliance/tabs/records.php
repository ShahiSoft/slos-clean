...
$available_regions = array(
	...
	'NA'     => __( 'North America', 'shahi-legalflowsuite' ),
	'LATAM'  => __( 'Latin America', 'shahi-legalflowsuite' ),
	'APAC'   => __( 'Asia Pacific', 'shahi-legalflowsuite' ),
	'GLOBAL' => __( 'Global (Other)', 'shahi-legalflowsuite' ),
);
?>
...
<div class="slos-records-container">
	...
	<p class="slos-section-description">
		<?php esc_html_e( 'Complete audit trail of all consent interactions on your site. This data serves as proof of consent for GDPR Article 7 compliance. Use filters to narrow results or search by user identifier. Export records for compliance audits and regulatory inquiries.', 'shahi-legalflowsuite' ); ?>
	</p>
	...
	<div class="slos-records-toolbar">
		<div class="slos-filters-row">
			<select class="slos-filter-select" id="filter-type">
				<option value=""><?php esc_html_e( 'All Types', 'shahi-legalflowsuite' ); ?></option>
				<option value="necessary" <?php selected( $filter_type, 'necessary' ); ?>><?php esc_html_e( 'Necessary', 'shahi-legalflowsuite' ); ?></option>
				<option value="analytics" <?php selected( $filter_type, 'analytics' ); ?>><?php esc_html_e( 'Analytics', 'shahi-legalflowsuite' ); ?></option>
				<option value="marketing" <?php selected( $filter_type, 'marketing' ); ?>><?php esc_html_e( 'Marketing', 'shahi-legalflowsuite' ); ?></option>
				<option value="preferences" <?php selected( $filter_type, 'preferences' ); ?>><?php esc_html_e( 'Preferences', 'shahi-legalflowsuite' ); ?></option>
			</select>

			<select class="slos-filter-select" id="filter-status">
				<option value=""><?php esc_html_e( 'All Statuses', 'shahi-legalflowsuite' ); ?></option>
				<option value="accepted" <?php selected( $filter_status, 'accepted' ); ?>><?php esc_html_e( 'Accepted', 'shahi-legalflowsuite' ); ?></option>
				<option value="rejected" <?php selected( $filter_status, 'rejected' ); ?>><?php esc_html_e( 'Rejected', 'shahi-legalflowsuite' ); ?></option>
				<option value="withdrawn" <?php selected( $filter_status, 'withdrawn' ); ?>><?php esc_html_e( 'Withdrawn', 'shahi-legalflowsuite' ); ?></option>
			</select>

			<select class="slos-filter-select" id="filter-date">
				<option value="7d" <?php selected( $filter_date, '7d' ); ?>><?php esc_html_e( 'Last 7 days', 'shahi-legalflowsuite' ); ?></option>
				<option value="30d" <?php selected( $filter_date, '30d' ); ?>><?php esc_html_e( 'Last 30 days', 'shahi-legalflowsuite' ); ?></option>
				<option value="90d" <?php selected( $filter_date, '90d' ); ?>><?php esc_html_e( 'Last 90 days', 'shahi-legalflowsuite' ); ?></option>
				<option value="all" <?php selected( $filter_date, 'all' ); ?>><?php esc_html_e( 'All time', 'shahi-legalflowsuite' ); ?></option>
			</select>

			<?php if ( ! empty( $geo_rules ) ) : ?>
			<select class="slos-filter-select" id="filter-geo-rule">
				<option value=""><?php esc_html_e( 'All Geo Rules', 'shahi-legalflowsuite' ); ?></option>
				<?php
				foreach ( $geo_rules as $rule ) :
					$rule_id   = $rule['id'] ?? '';
					$rule_name = $rule['name'] ?? __( 'Unnamed Rule', 'shahi-legalflowsuite' );
					?>
			...
			<select class="slos-filter-select" id="filter-region">
				<option value=""><?php esc_html_e( 'All Regions', 'shahi-legalflowsuite' ); ?></option>
			...
			<input type="search" 
					...
					placeholder="<?php esc_attr_e( 'Search by user, email, or ID...', 'shahi-legalflowsuite' ); ?>"
		...
		<div class="slos-bulk-actions">
			<span class="slos-selected-count" id="selected-count" style="display: none;">
				<strong>0</strong> <?php esc_html_e( 'selected', 'shahi-legalflowsuite' ); ?>
			</span>
			<button class="slos-btn slos-btn-secondary" id="bulk-export" style="display: none;">
				<span class="dashicons dashicons-download"></span>
				<?php esc_html_e( 'Export Selected', 'shahi-legalflowsuite' ); ?>
			</button>
		...
		<table class="slos-records-table" id="consent-records-table">
			...
			<tbody id="records-tbody">
				<?php if ( ! empty( $recent_activity ) ) : ?>
					<?php
					foreach ( $recent_activity as $consent ) :
						...
						<tr data-id="<?php echo esc_attr( $consent_arr['id'] ?? 0 ); ?>">
							...
							<td>
								<div class="slos-user-cell">
									...
									<div class="slos-user-info">
										<div class="name"><?php echo esc_html( $user ? $user->display_name : __( 'Guest User', 'shahi-legalflowsuite' ) ); ?></div>
										<div class="email"><?php echo esc_html( $user ? $user->user_email : __( 'Anonymous', 'shahi-legalflowsuite' ) ); ?></div>
									</div>
								</div>
							</td>
							...
							<td>
								<div class="slos-source-icon banner" title="<?php esc_attr_e( 'Consent Banner', 'shahi-legalflowsuite' ); ?>">
							...
							<td>
								<div class="slos-actions-cell">
									<button class="slos-action-icon" data-action="view" data-id="<?php echo esc_attr( $consent_arr['id'] ?? 0 ); ?>" title="<?php esc_attr_e( 'View Details', 'shahi-legalflowsuite' ); ?>">
										<span class="dashicons dashicons-visibility"></span>
									</button>
									<button class="slos-action-icon" data-action="export" data-id="<?php echo esc_attr( $consent_arr['id'] ?? 0 ); ?>" title="<?php esc_attr_e( 'Export', 'shahi-legalflowsuite' ); ?>">
										<span class="dashicons dashicons-download"></span>
									</button>
									<button class="slos-action-icon" data-action="history" data-id="<?php echo esc_attr( $consent_arr['id'] ?? 0 ); ?>" title="<?php esc_attr_e( 'View History', 'shahi-legalflowsuite' ); ?>">
					...
					<tr>
						<td colspan="9">
							<div class="slos-empty-state">
								...
								<p><?php esc_html_e( 'No consent records found. Records will appear here as users interact with your consent banner.', 'shahi-legalflowsuite' ); ?></p>
							</div>
						</td>
					</tr>
				<?php endif; ?>
		...
		<div class="slos-pagination">
			...
			<div class="slos-pagination-controls">
				<div class="slos-per-page">
					<label><?php esc_html_e( 'Per page:', 'shahi-legalflowsuite' ); ?></label>
					<select id="per-page">
...
jQuery(document).ready(function($) {
	...
	function viewConsentHistory(id) {
		$.ajax({
			...
			success: function(response) {
				...
				if (logs.length > 0) {
					let history = '<?php echo esc_js( __( 'History for Consent #', 'shahi-legalflowsuite' ) ); ?>' + id + ':\n\n';
					logs.forEach(function(log, i) {
						history += (i + 1) + '. ' + (log.action || 'Action') + ' - ' + (log.created_at || 'Date') + '\n';
					});
					alert(history);
				} else {
					alert('<?php echo esc_js( __( 'No history found for this consent.', 'shahi-legalflowsuite' ) ); ?>');
				}
			},
			error: function() {
				alert('<?php echo esc_js( __( 'Error loading consent history.', 'shahi-legalflowsuite' ) ); ?>');
			}
		});
	}
	...
	function loadRecords(sortOpts) {
		...
		$tbody.html('<tr><td colspan="9" style="text-align: center; padding: 40px;"><span class="dashicons dashicons-update" style="animation: spin 1s linear infinite;"></span> <?php echo esc_js( __( 'Loading...', 'shahi-legalflowsuite' ) ); ?></td></tr>');
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (records.length === 0) {
					$tbody.html('<tr><td colspan="9"><div class="slos-empty-state"><span class="dashicons dashicons-list-view"></span><p><?php echo esc_js( __( 'No records found matching your filters.', 'shahi-legalflowsuite' ) ); ?></p></div></td></tr>');
				...
				records.forEach(function(record) {
					...
					html += '<button class="slos-action-icon" data-action="view" data-id="' + record.id + '" title="<?php echo esc_js( __( 'View Details', 'shahi-legalflowsuite' ) ); ?>"><span class="dashicons dashicons-visibility"></span></button>';
					html += '<button class="slos-action-icon" data-action="export" data-id="' + record.id + '" title="<?php echo esc_js( __( 'Export', 'shahi-legalflowsuite' ) ); ?>"><span class="dashicons dashicons-download"></span></button>';
					html += '<button class="slos-action-icon" data-action="history" data-id="' + record.id + '" title="<?php echo esc_js( __( 'View History', 'shahi-legalflowsuite' ) ); ?>"><span class="dashicons dashicons-backup"></span></button>';
					html += '</div></td></tr>';
				});
			...
			error: function() {
				$tbody.html('<tr><td colspan="9"><div class="slos-empty-state"><span class="dashicons dashicons-warning"></span><p><?php echo esc_js( __( 'Error loading records. Please try again.', 'shahi-legalflowsuite' ) ); ?></p></div></td></tr>');
			}
		});
	}
	...
	function updatePagination(total, showing) {
		...
		$('.slos-pagination-info').text('<?php echo esc_js( __( 'Showing', 'shahi-legalflowsuite' ) ); ?> ' + start + ' - ' + end + ' <?php echo esc_js( __( 'of', 'shahi-legalflowsuite' ) ); ?> ' + total + ' <?php echo esc_js( __( 'records', 'shahi-legalflowsuite' ) ); ?>');
		
		let paginationHtml = '<div class="slos-per-page"><label><?php echo esc_js( __( 'Per page:', 'shahi-legalflowsuite' ) ); ?></label><select id="per-page"><option value="25"' + (perPage === 25 ? ' selected' : '') + '>25</option><option value="50"' + (perPage === 50 ? ' selected' : '') + '>50</option><option value="100"' + (perPage === 100 ? ' selected' : '') + '>100</option></select></div>';
